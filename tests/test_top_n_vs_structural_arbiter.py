...
def test_top_n_wins_when_both_fire(prompt: str, overridden: str) -> None:
    ...
    assert _classify_structural_subtype(prompt) == overridden, (
        f"fixture stale: {prompt!r} structural subtype changed"
    )
    ...
    assert _OVERRIDE_NOTE in rejected, (
        f"override of {overridden} not disclosed for {prompt!r}; rejected={rejected!r}"
    )
