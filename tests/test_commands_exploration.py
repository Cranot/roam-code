...
# W414: All exploration-command tests in this file are read-only against
# the indexed project (they invoke search/grep/file/symbol/trace/deps/
# uses/fan/impact and validate output). Re-indexing on every test (the
# conftest function-scoped `indexed_project`) costs ~2s x 80 = ~3 minutes
# serial. Override at module scope here so the project is built and
# indexed once per worker. Mirrors W346's pattern in test_json_contracts.
@pytest.fixture(scope="module")
def indexed_project(tmp_path_factory):
    """Module-scoped indexed Python project for read-only exploration tests."""
    import textwrap

    proj = tmp_path_factory.mktemp("commands_exploration_proj")
