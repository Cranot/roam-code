...
def test_pass102_render_helpers_extracted():
    """Pass 102 — context helpers exist."""
...
def test_pass104_doctor_command_registry_loads():
    """Pass 104 — doctor still validates the command registry after the cycle break."""
    runner = CliRunner()
...
def test_pass105_health_classifies_infrastructure_as_utility():
    """Pass 105 — cli.py / mcp_server.py / graph/ counted as utilities."""
...
def test_pass106_dataflow_dead_helpers_extracted():
    """Pass 106 — dataflow analyzer split into per-pattern helpers."""
...
def test_pass108_test_map_unknown_symbol_emits_json():
    """Pass 108 — test-map for a missing symbol returns a JSON envelope."""
    runner = CliRunner()
...
def test_pass110_orphan_imports_no_false_internal_typos():
    """Pass 110 — newly-added modules don't get flagged as internal_typo."""
    runner = CliRunner()
...
def test_pass110_modules_from_path_helper():
    """Pass 110 — _modules_from_path adds every dotted prefix."""
