...
@pytest.mark.parametrize("filename", ["app.py", "δοκιμή.py"], ids=["ascii", "unicode"])
def test_mcp_child_uses_roam_not_project_module(tmp_path, monkeypatch, shadow_source, operation, filename):
    ...
    git_init(root)
    # Exercise Git's default quoting policy, independent of the host's settings.
    ...
    if operation == "search":
        ...
        assert result["results"][0]["location"] == f"{filename}:1"
    else:
        ...
        assert [item["file_path"] for item in result["changed_symbols"]] == [filename]
