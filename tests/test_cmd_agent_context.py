...
def test_agents_below_range_rejected(agent_project, runner):
    result = invoke_cli(
        runner, ["agent-context", "--agent-id", "1", "--agents", "0"], cwd=agent_project
    )
    assert result.exit_code == 2
    assert "range x>=1" in result.output
...
def test_json_write_files_nonempty_and_lists_typed(agent_project, runner):
    ...
    for list_key in ("read_only_dependencies", "interface_contracts", "depends_on_partitions",
                     "downstream_partitions", "key_symbols"):
...
def test_not_found_text_exits_one_with_guidance(agent_project, runner):
    result = invoke_cli(
        runner, ["agent-context", "--agent-id", "9", "--agents", "2"], cwd=agent_project
    )
...
def test_not_found_default_agents_uses_max(agent_project, runner):
    ...
    result, data = _run_json_raw(
        runner, ["agent-context", "--agent-id", "5"], cwd=agent_project
    )
    assert result.exit_code == 1
...
def test_text_output_structure(agent_project, runner):
    result = invoke_cli(
        runner, ["agent-context", "--agent-id", "1", "--agents", "3"], cwd=agent_project
    )
