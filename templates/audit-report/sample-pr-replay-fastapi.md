...
**Verdict:** 1 of 30 PRs (3%) would have surfaced findings — 0 review-eligible (high), 2 review-required (medium).

- PRs replayed: **30**
- PRs Roam would have flagged pre-merge: **1**
- High-severity findings (would block CI): **0**
- Medium-severity findings (would gate review): **2**
...
The highest-impact class on this window was **`impact`** (2 findings across 1 PRs). Wiring a CI gate against this class is the single highest-leverage move surfacing from this replay.
...
|---|---|---|---:|---:|---|
| 2026-07-01 | `319be50` | ✨ Support dependencies in `app.frontend()`, e.g. for automat | 0 | 2 | impact x2 |

## Recommended next steps

- **Wire CI gates against the top 1 detector class(es)** — `impact`. `roam critique` returns exit code 5 on any high-severity finding, so a single CI step gates every PR. See <https://roam-code.com/docs/>.
...
_Generated by `roam pr-replay --tier team` on 2026-07-06 20:47 UTC. Engine: `roam postmortem` walks the range; `roam critique` evaluates each diff. Both ship in the open-source CLI ([github.com/Cranot/roam-code](https://github.com/Cranot/roam-code))._
