...
def _aggregate_scalars(
    ...
    for field in fields:
        ...
        if field_value is None or isinstance(field_value, _SAFE_SCALAR_TYPES):
            if field in source:
                out[field] = field_value[:500] if isinstance(field_value, str) else field_value
    return out


def _closed_state_counts(
    rows: object,
    field: str,
    ...
    for row in rows if isinstance(rows, list) else []:
        value = str(row.get(field) or "unknown") if isinstance(row, dict) else "unknown"
        counts[value if value in allowed else "unknown"] += 1
    return dict(sorted(counts.items(), key=lambda item: (-item[1], item[0])))


def aggregate_savings_payload(payload: dict[str, Any]) -> dict[str, Any]:
    """Return the browser-safe savings contract without transcript-derived values.

    Only scalar coverage, fixed verdict vocabulary, and counts cross this
    boundary. Pattern text, task prefixes, commands, paths, identifiers, and
    per-episode rows are deliberately unreachable from the returned object.
    """
    atlas = payload.get("procedure_atlas") if isinstance(payload.get("procedure_atlas"), dict) else {}
    intervention = (
        payload.get("intervention_evidence") if isinstance(payload.get("intervention_evidence"), dict) else {}
    )
    assignments = intervention.get("assignments") if isinstance(intervention.get("assignments"), list) else []
    experiments = intervention.get("experiments") if isinstance(intervention.get("experiments"), list) else []
    mappings = atlas.get("intervention_mappings") if isinstance(atlas.get("intervention_mappings"), list) else []
    summary = _aggregate_scalars(payload.get("summary"), _AGGREGATE_SUMMARY_FIELDS)
    summary["causal_savings_claimed"] = False
    return {
        "summary": summary,
        "coverage": _aggregate_scalars(
            payload.get("coverage"),
            _AGGREGATE_COVERAGE_FIELDS,
        ),
        "sensor_canaries": _aggregate_scalars(
            payload.get("sensor_canaries"),
            ("state", "passed", "total"),
        ),
        "opportunity_counts": {
            "repeated_live_candidates": len(payload.get("repeat_candidates") or []),
            "historical_pattern_candidates": len(payload.get("historical_candidates") or []),
            "ranked_work_opportunities": len(atlas.get("opportunities") or []),
            "failure_signatures": len(atlas.get("failure_signatures") or []),
            "recovery_targets": len(atlas.get("recovery_targets") or []),
            "intervention_mappings": len(mappings),
        },
        "intervention_state": {
            "declaration_states": _closed_state_counts(
                mappings,
                "declaration_state",
                frozenset({"declared_native", "declared_partial", "unclaimed"}),
            ),
            "assignments": len(assignments),
            "experiments": len(experiments),
            "assignment_states": _closed_state_counts(
                assignments,
                "assignment",
                frozenset({"control", "exposed", "shadow"}),
            ),
            "causal_savings_claimed": False,
        },
        "privacy": {
