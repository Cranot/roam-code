...
# Pass 79 — deprecated commands map to their replacement.  When a user
...
_COMMANDS = {
    ...
    # Aliases for the consolidated trends command (round 4 #16). Older
...
class LazyGroup(click.Group):
    ...
    def get_command(self, ctx, cmd_name):
        # V1 (2026-05-06) — built-ins resolve without paying the
    ...
    def resolve_command(self, ctx, args):
        ...
        Pass 79 — also surface a deprecation note on stderr when the
        invoked command is in ``_DEPRECATED_COMMANDS`` so users know
        about a planned rename / replacement.
        """
        # Pass 79 — pre-resolve deprecation hint.
        if args:
            cmd_name = args[0]
        ...
        try:
            ...
            # Pass 41 — when no edit-distance match lands but the user
...
# V6 (2026-05-06) — `_short_help_via_ast` is called 126x by `roam --help`,
...
def cli(ctx, json_mode, compact, agent, sarif_mode, budget, include_excluded, detail):
    ...
    # Pass 42 — opt-in local telemetry. Records (cmd, duration_ms,
