...
# ---- installed-tool version stamp for cache keys --------------------------
#
# Every on-disk cache in `.roam/compile-envelope-cache.sqlite` (envelopes,
# plans, probe results) is produced BY THE TOOL, so the tool's own version
# belongs in the key. Without it, upgrading roam in place leaves the target
# repo's HEAD untouched — nothing else in the key moves — and the new code
# serves envelopes generated by the OLD code while every surface reports
# success. Measured 2026-08-04: released 13.9.0 stored an envelope that
# released 13.10.0 read back verbatim (identical key) once file mtimes were
# normalized, and the obligations/execution-contract block that the compiler
# emits today does not exist in any released version — so a cross-version hit
# silently drops it.
#
# Why not `roam.__version__`: it resolves through `importlib.metadata`, which
# costs ~90ms on Windows (measured) — the exact cost `roam/__init__.py`
# defers so the per-prompt compile hook never pays it. A cache HIT is ~5ms,
# so importing metadata to protect it would cost 18x the thing protected.
# The dist-info directory name beside the installed package carries the same
# version for one `os.scandir` (~0.13ms measured, memoized per process).
_ROAM_VERSION_STAMP: str | None = None


def _version_stamp_from_site_dir(site_dir: str) -> str:
    """The `roam_code-<version>.dist-info` name found in ``site_dir``, or "".

    Split out from ``_roam_version_stamp`` so the discovery itself is testable
    against a synthetic install layout — a stamp helper that silently returned
    "" everywhere would disarm every cache key that folds it in.
    """
    try:
        with os.scandir(site_dir) as entries:
            for entry in entries:
                name = entry.name
                if not name.startswith(("roam_code-", "roam-code-")):
                    continue
                if name.endswith(".dist-info") or name.endswith(".egg-info"):
                    return name
    except OSError as exc:
        # Unreadable site dir: fall back to the empty stamp (pre-existing
        # behaviour) rather than breaking every cache read.
        log_swallowed("plan_cache.version_stamp", exc)
    return ""


def _roam_version_stamp() -> str:
    """Installed roam-code version for cache keys; "" when undeterminable.

    Reads `<name>-<version>.dist-info` (or `.egg-info`) beside the installed
    package. Returns "" for a source checkout / editable install, where no
    dist-info sits next to the package — those are dev trees, whose staleness
    is already covered by the compiler-file mtime stamp that moves on every
    edit. Memoized: the installed version cannot change inside a process.
    """
    global _ROAM_VERSION_STAMP
    if _ROAM_VERSION_STAMP is not None:
        return _ROAM_VERSION_STAMP
    # __file__ = <site-packages>/roam/plan/plan_cache.py
    site_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    _ROAM_VERSION_STAMP = _version_stamp_from_site_dir(site_dir)
    return _ROAM_VERSION_STAMP
...
def _run_roam_persist_key(args: list[str], cwd_norm: str) -> str:
    # The tool version is part of the key: these rows are `roam <subcmd>
    # --json` payloads, so an upgrade that changes a probe's output shape
    # must not read back the previous version's rows (HEAD does not move on
    # an upgrade, and the 24h TTL is far longer than an upgrade cycle).
    blob = "\x1f".join([cwd_norm or "", _roam_version_stamp(), *args]).encode("utf-8", "replace")
