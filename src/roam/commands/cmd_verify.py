...
# ---------------------------------------------------------------------------
# Fabricated-success check (opt-in)
# ---------------------------------------------------------------------------


def _check_fabricated_success(conn, file_ids: list[int]) -> dict:
    """Flag external-sink functions that return success without an effect."""
    if not file_ids:
        return {"score": 100, "violations": []}
    ...
    violations: list[dict] = []
    for finding in classify_fabricated_success(conn):
        if (finding.file or "").replace("\\", "/") not in scope:
            continue
        message = (
            f"fabricated success: {finding.symbol} declares a {finding.declared_sink} sink "
            f"but returns {finding.success_shape} without an external effect"
        )
        violation = finding.to_dict()
        violation.update(
            {
                "category": _VERIFY_FABRICATED_SUCCESS_CATEGORY,
                "severity": SEVERITY_WARN,
                "file": finding.file,
                "line": finding.line_start or finding.line_end or 1,
                "message": message,
...
def _check_return_in_finally(conn, file_ids: list[int]) -> dict:
    ...
    violations: list[dict] = []
    for finding in classify_return_in_finally(conn):
        if (finding.file or "").replace("\\", "/") not in scope:
            continue
        message = f"exception-swallowing control flow: {finding.symbol} has a {finding.statement_kind} inside finally"
        violation = finding.to_dict()
        violation.update(
            {
                "category": _VERIFY_RETURN_IN_FINALLY_CATEGORY,
                "severity": SEVERITY_WARN,
                "file": finding.file,
                "line": finding.line_start or finding.line_end or 1,
                "message": message,
                "fix": (
                    f"Move the {finding.statement_kind} outside the finally block in `{finding.symbol}`, "
                    "or preserve the active exception before exiting"
                ),
            }
        )
        violations.append(violation)
...
# ---------------------------------------------------------------------------
# Unreachable-after-return check (opt-in)
# ---------------------------------------------------------------------------


def _check_unreachable_after_return(conn, file_ids: list[int]) -> dict:
    """Flag statements that follow an unconditional terminator in a block."""
    if not file_ids:
        return {"score": 100, "violations": []}
    ...
    violations: list[dict] = []
    for finding in classify_unreachable_after_return(conn):
        if (finding.file or "").replace("\\", "/") not in scope:
            continue
        message = (
            f"unreachable code: {finding.symbol} has a statement after {finding.terminator} that can never execute"
        )
        violation = finding.to_dict()
        violation.update(
            {
                "category": _VERIFY_UNREACHABLE_AFTER_RETURN_CATEGORY,
                "severity": SEVERITY_WARN,
                "file": finding.file,
                "line": finding.dead_line or finding.line_start or finding.line_end or 1,
                "message": message,
                "fix": "remove unreachable code after the `return`/`raise` (it can never execute)",
            }
        )
        violations.append(violation)
...
def _run_verify_categories(conn, selected: list[str], file_ids: list[int], target_paths: list[str], root: Path) -> dict:
    return {
        ...
        _VERIFY_RETURN_IN_FINALLY_CATEGORY: _maybe_run_verify_check(
            selected,
            _VERIFY_RETURN_IN_FINALLY_CATEGORY,
            lambda: _check_return_in_finally(conn, file_ids),
        ),
        _VERIFY_FABRICATED_SUCCESS_CATEGORY: _maybe_run_verify_check(
            selected,
            _VERIFY_FABRICATED_SUCCESS_CATEGORY,
        ...
        _VERIFY_UNREACHABLE_AFTER_RETURN_CATEGORY: _maybe_run_verify_check(
            selected,
            _VERIFY_UNREACHABLE_AFTER_RETURN_CATEGORY,
