...
@click.option(
    "--since-tag",
    is_flag=True,
    help="Pass 64 — analyze commits since the most recent tag (auto-detected via git describe).",
)
@click.pass_context
def diff_cmd(ctx, commit_range, staged, full, tests, coupling, fitness, since_tag):
    ...
    # Pass 64 — --since-tag auto-fills commit_range with <last-tag>..HEAD.
    ...
    with open_db(readonly=True) as conn:
        ...
        # W5 (2026-05-06) — surface the top-3 affected symbols by PageRank
