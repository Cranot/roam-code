...
def _format_explanation_text(expl: dict) -> list[str]:
    ...
    if bm25 is not None and pagerank is not None:
        # Pass 46 — surface the structural boost so the user can see
...
@click.option(
    ...
    show_default=True,
    help=(
        "Pass 59 — match style: ``substring`` (default, LIKE %p%), "
...
@click.option(
    ...
    default=0,
    show_default=True,
    help="Pass 72 — boost results in files modified within N days (0 = no boost).",
)
@click.pass_context
def search(ctx, pattern, full, kind_filter, async_only, decorator_filter, fixtures_only, explain, mode, recent_days):
    ...
    with open_db(readonly=True) as conn:
        # Pass 59 — register a REGEXP function so SQLite can route
        ...
        # Pass 72 — recency boost: when --recent N is set, add a
        ...
        if explain:
            for r in rows:
                ...
                # Pass 46 — augment with the per-result PageRank so the
        ...
        for r in rows:
            refs = ref_counts.get(r["id"], 0)
            pr = r["pagerank"] or 0
            # Per dogfood notes 2026-05-01: 4-decimal PR rounded all
