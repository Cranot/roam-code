...
def search_cmd(
    ...
    with open_db(readonly=True) as conn:
        ...
        if explain:
            ...
            explanations = _get_explain_data_batch(
                conn, [r["id"] for r in rows], pattern, warnings_out=warnings_out
            )
            # augment with the per-result PageRank so the user can see
            # structural boost contribution alongside BM25.
            for r in rows:
                explanations[r["id"]]["pagerank"] = (
                    round(r["pagerank"], 6) if r["pagerank"] else 0
                )
