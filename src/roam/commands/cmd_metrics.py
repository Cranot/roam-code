...
def _collect_file_symbol_metrics_bulk(
    ...
    if not joined:
        ...
        zero_symbols = [
            sid for sid in symbol_ids if by_id[sid]["fan_in"] == 0 and by_id[sid]["fan_out"] == 0
        ]
        if zero_symbols:
            try:
                fi_map = {
                    ...
                    for row in batched_in(
                        ...
                        "WHERE target_id IN ({ph}) GROUP BY target_id",
                        zero_symbols,
                    )
                ...
                fo_map = {
                    ...
                    for row in batched_in(
                        ...
                        "WHERE source_id IN ({ph}) GROUP BY source_id",
                        zero_symbols,
                    )
