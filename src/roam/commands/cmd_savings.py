...
SARIF is deliberately NOT emitted because savings evidence is episode- and
experiment-scoped rather than a file-located code finding. There are no source
coordinates that could populate SARIF ``locations[]`` without inventing them.
"""
...
@click.option(
    "--aggregate",
    is_flag=True,
    help="Emit only browser-safe scalar coverage and counts; excludes pattern and episode values.",
)
@click.pass_context
@roam_capability(
    name="savings",
...
def savings(ctx: click.Context, root: str, schema: bool, aggregate: bool) -> None:
    ...
    token_budget = int(ctx.obj.get("budget", 0) or 0) if ctx.obj else 0
    if schema:
        if json_mode:
            click.echo(
                to_json(
    ...
    if aggregate:
        aggregate_payload = aggregate_savings_payload(result)
        aggregate_summary = aggregate_payload.pop("summary")
        click.echo(
            to_json(
                json_envelope(
                    "savings",
                ...
                json_envelope(
                    ...
                    agent_contract={
                        "facts": [
                            ...
                            f"{coverage.get('terminal_outcomes', 0)} terminal outcome records",
                            f"{coverage.get('fully_joined_episodes', 0)} fully joined event records",
                            f"{len(result.get('historical_candidates', []))} historical candidate patterns",
                            f"{len((result.get('procedure_atlas') or {}).get('opportunities', []))} ranked work findings",
                            f"{sum(row.get('declaration_state') == 'unclaimed' for row in (result.get('procedure_atlas') or {}).get('intervention_mappings', []))} unclaimed intervention capabilities",
