...
@click.option(
    ...
    type=str,
    default=None,
    help="Pass 65 — limit tour items (top symbols, reading order, entries) to files under this path prefix.",
)
@click.pass_context
def tour(ctx, write_file, mermaid_mode, focus_path):
    ...
    with open_db(readonly=True) as conn:
        ...
        # Pass 65 — focus filter. Apply after the heavy queries so we
