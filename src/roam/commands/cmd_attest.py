...
def _append_attestation_verdict_section(lines, plan, is_md):
    if is_md:
...
def _append_attestation_risk_section(lines, plan, is_md):
    risk = plan["risk"]
    if risk:
        if is_md:
...
def _append_attestation_blast_section(lines, plan, is_md):
    blast = plan["blast"]
    if is_md:
...
def _append_attestation_breaking_section(lines, plan, is_md):
...
def _append_attestation_budget_section(lines, plan, is_md):
    budget = plan["budget"]
    if plan["has_budget"]:
        if is_md:
            lines.append(
                f"### Budget ({budget['passed']} passed, {budget['failed']} failed, {budget['skipped']} skipped)"
            )
            lines.append("")
...
def _append_attestation_fitness_section(lines, plan, is_md):
    if not is_md:
...
def _append_attestation_tests_section(lines, plan, is_md):
    tests = plan["tests"]
    if plan["has_tests"]:
        if is_md:
        ...
        if is_md:
            pass  # markdown omits the tests section when none
        else:
            lines.append("AFFECTED TESTS: none found")
            lines.append("")


def _append_attestation_effects_section(lines, plan, is_md):
    if plan["has_effects"]:
        effect_items = plan["effect_items"]
        limit = 20 if is_md else 15
        if is_md:
            lines.append(f"### Effects ({len(effect_items)} symbols)")
            lines.append("")
            for e, all_eff in effect_items[:limit]:
...
def _append_attestation_metadata_section(lines, plan, is_md):
    if is_md:
        lines.append("---")
...
def _render_attestation_report(plan, fmt):
    """Build attestation report lines for *fmt* (``markdown`` or ``text``).

    Centralizes the evidence-section ordering and content decisions so the
    markdown and text formatters cannot drift. Format-specific branches are
    limited to cosmetic tokens.
    """
    is_md = fmt == "markdown"
    lines = []
    for append_section in (
        _append_attestation_verdict_section,
        _append_attestation_risk_section,
        _append_attestation_blast_section,
        _append_attestation_breaking_section,
        _append_attestation_budget_section,
        _append_attestation_fitness_section,
        _append_attestation_tests_section,
        _append_attestation_effects_section,
        _append_attestation_metadata_section,
    ):
        append_section(lines, plan, is_md)
