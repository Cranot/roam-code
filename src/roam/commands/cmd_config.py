...
def _validate_config(root: Path, current: dict, json_mode: bool) -> None:
    """Pass 55 — flag unknown keys / type mismatches in .roam/config.json."""
    issues: list[dict] = []
...
def _show_rerank_weights(root: Path, json_mode: bool) -> None:
    """Pass 73 — print the active rerank weights merged with defaults."""
...
def _show_env_inventory(json_mode: bool) -> None:
    """Pass 36 — enumerate ROAM_* environment variables read by the codebase.
...
@click.option(
    ...
    "show_weights",
    is_flag=True,
    help="Pass 73 — print the active rerank weights (alpha/beta/gamma/delta/epsilon/zeta).",
)
@click.pass_context
def config(
    ctx,
