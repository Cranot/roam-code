...
# E5 (2026-05-06) — body-level signals that the method shapes its output
...
def analyze_over_fetch(conn, threshold: int, limit: int) -> list[dict]:
    ...
    for model_info in model_files:
        ...
        if fillable_count >= 30 and hidden_count == 0 and not has_resource:
            ...
            # W2 (2026-05-06) — when the model is *huge* (>=50 fields) and
        ...
        # U4 (2026-05-06) — `matched_patterns` mirrors what _io_emit_finding
...
def over_fetch_cmd(ctx, threshold, limit):
    ...
    for f in findings:
        ...
        for reason in f["reasons"]:
            click.echo(f"          {reason}")
        # U7 (2026-05-06) — surface matched_patterns when present so the text
