...
Pass 44 — quick lint that catches:
...
Pass 96 — extended to JS / TS (``import 'x'``) and Go (``import "x"``).
...
def _indexed_python_modules(conn) -> set[str]:
    ...
    Pass 110 — also walks ``src/`` directly (filesystem) so modules
    ...
    # Pass 110 — filesystem fallback for newly-added files.
    src_root = Path("src")
    if src_root.is_dir():
