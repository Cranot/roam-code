...
def _betweenness_percentiles(conn, percentiles=(70, 90)):
    ...
    needs, then SELECT only those rows positionally via ``ORDER BY`` +
    ``LIMIT``/``OFFSET``. Returns a ``BetweennessPercentiles`` whose ``values``
    list aligns with ``percentiles`` (floats; 0 when the population is empty)
    and whose ``population`` is the positive-betweenness row count. Without a
    betweenness index the per-query cost is a partial scan, but row
    materialization/transfer + the Python sort are eliminated — the dominant
    cost on large codebases.
    """
    n = conn.execute(
        "SELECT COUNT(*) FROM graph_metrics WHERE betweenness > 0"
    ).fetchone()[0]
    if n == 0:
