...
def _build_review_suggestions(
    *,
    ...
    if rule_bodies:
        ...
        suggested_yaml = header + "".join(rule_bodies)
    else:
        suggested_yaml = None
    ...
    for row in recurring:
        ...
        if gate in seen_gates:
            # De-duplicate when two detectors share a gate (rare, but possible).
            continue
        seen_gates.add(gate)
        ci_gates.append(
            {
    ...
    # Always include the umbrella gate — it covers everything critique knows about.
    # We append (not prepend) so detector-specific gates lead.
    ...
    for c in commits:
        ...
        # The rationale cites detector classes by name so the buyer can
        # trace the block back to the same row in recurring_risk_classes.
        top_kinds = ", ".join(kinds[:3]) if kinds else "high-severity finding"
        blocked.append(
            {
                "sha": c.get("short_sha") or c.get("sha"),
                "date": c.get("date"),
    ...
    # Cap at 10 — the per-PR section already lists everything; this block
    # is the "what would the gate have done?" shortlist.
    blocked = blocked[:10]
...
def _gather_rules_envelopes(active_run_id: str | None, warnings: list[str]) -> list[dict]:
    ...
    if active_run_id:
        ...
        if env_dir.is_dir():
            try:
                for f in sorted(env_dir.glob("rules-*.json")):
                    try:
...
def _gather_context_files(
    ...
    for line in text.splitlines():
        ...
        # Bound the artifact list so a 10k-file refactor doesn't bloat
...
def _gather_constitution_policy_decisions(
    ...
    *evaluation result* is captured elsewhere by the gate's own command).
    ...
    # channel attribution. ``producer_envelope(constitution)`` names the
...
def _gather_permit_policy_decisions(
    ...
    try:
        ...
        warnings.append(f"permits: project_root_lookup_failed — find_project_root raised {type(exc).__name__}: {exc}")
        return []
    if repo_root is None:
        warnings.append(
            "permits: project_root_not_found — find_project_root returned None (no .git ancestor; not a roam project)"
        )
        return []
...
def _gather_lease_policy_decisions(
    ...
    try:
        ...
        warnings.append(f"leases: project_root_lookup_failed — find_project_root raised {type(exc).__name__}: {exc}")
        return []
    if repo_root is None:
        warnings.append(
            "leases: project_root_not_found — find_project_root returned None (no .git ancestor; not a roam project)"
        )
        return []
    leases_dir = Path(repo_root) / ".roam" / "leases"
    if not leases_dir.is_dir():
        ...
        # operationally surprising — emit an info marker so the operator
        # has a breadcrumb for "why aren't my leases showing?". Default /
        # ``safe_edit`` / ``read_only`` runs stay silent; W267's
        # ``test_pr_replay_gathers_handle_missing_state`` asserts the
    ...
    * ``source_was_provided``  - ``True`` iff the caller passed one of the
    ...
    try:
        if fixture_path is not None:
            reviews = load_reviews_from_fixture(Path(fixture_path))
        else:
            owner, repo, gh_pr = _parse_gh_spec(gh_spec)
...
def _render_evidence_markdown(
    *,
    ...
    # W259 — honest evidence-coverage banner. First signal a reviewer
    # sees, above every other section, so over-claiming is caught at
    # the top of the report rather than after they read several
    # populated tables. Banner threshold table lives in
    # ``roam.evidence.banner`` (W254 measurement -> W259 implementation).
    ...
    if not evidence.changed_subjects:
        ...
        for subj in evidence.changed_subjects[:20]:
            ...
            # W217 hostile-input safety: subject qualified_name can contain
            # arbitrary user-controlled bytes; sanitise before backticking.
    ...
    # Per the crosswalk memo (dev/AGENTIC-ASSURANCE-CROSSWALK-2026-05-13.md),
    ...
    out.append("|---|---|---|")
    # Roll up findings → (detector, confidence) → count.
    pair_counts: dict[tuple[str, str], int] = {}
    for f in evidence.findings:
        det = str(f.get("detector") or "unknown")
        conf = str(f.get("confidence") or f.get("source") or "aggregate")
        key = (det, conf)
        pair_counts[key] = pair_counts.get(key, 0) + int(f.get("total_findings", 1) or 1)
    if not pair_counts:
        out.append("| _(none)_ | — | 0 |")
    else:
        for (det, conf), count in sorted(pair_counts.items(), key=lambda kv: -kv[1])[:20]:
            # W217 hostile-input safety: detector / confidence come from
            # detector-produced findings rows; sanitise before rendering.
    ...
    if not evidence.approvals and not evidence.accepted_risks:
        ...
        if evidence.approvals:
            ...
            for a in evidence.approvals[:10]:
                # W217 hostile-input safety: approvals are caller-supplied
                # Mappings; ``str(...)`` could embed newlines / pipes /
                # control chars that break the bullet structure.
                out.append(f"- {_escape_cell_text(str(a))}")
        if evidence.accepted_risks:
            out.append("")
            out.append("Accepted risks:")
            for r in evidence.accepted_risks[:10]:
                out.append(f"- {_escape_cell_text(str(r))}")
    out.append("")
    ...
    if review_suggestions is None:
        ...
        out.append("")
        recurring = review_suggestions.get("recurring_risk_classes") or []
        if not recurring:
        ...
        out.append("")
        rules_yaml = review_suggestions.get("suggested_roam_rules_yml")
        if rules_yaml:
            out.append("```yaml")
        ...
        out.append("")
        gates = review_suggestions.get("suggested_ci_gates") or []
        if gates:
            out.append("```bash")
            for g in gates[:5]:
                out.append(f"# {g.get('rationale', '')}")
                out.append(g.get("gate", ""))
            out.append("```")
        else:
        ...
        if not would_block:
            ...
            out.append("|---|---|---|---:|---|")
            for b in would_block[:5]:
                # W217 hostile-input safety: every cell sanitised before
                # rendering. SHA / date are backticked, so use code-cell
                # escape; subject / rationale are prose, so use text-cell
                # escape with link-injection protection.
                sha = _escape_cell_code(b.get("sha", "?"))
...
def _render_evidence_limitations(evidence) -> str:
    ...
    crosswalk memo (``dev/AGENTIC-ASSURANCE-CROSSWALK-2026-05-13.md``).
...
def _derive_limitations(evidence) -> tuple[str, ...]:
    ...
    # Source 1: Per-Q gaps from evidence_completeness().
    # We tolerate packets lacking the method (e.g. partial test
    # fixtures) by skipping this source — derivation is best-effort
    # at the section level, not a hard contract.
    completeness: dict | None = None
    if hasattr(evidence, "evidence_completeness"):
        try:
            completeness = evidence.evidence_completeness()
        except Exception:
            completeness = None
    ...
    if completeness:
        for q_num in range(1, 9):
            q_key = f"Q{q_num}"
            state = completeness.get(q_key)
            if state not in ("missing", "partial"):
                continue
            name, why = _Q_GAP_LABELS[q_key]
            if state == "partial":
                # Q8 + producer_not_available is the canonical partial
                # path today (W261). Surface that wording when present
                # so the reader sees the SAME explanation in both the
    ...
    # corroborating provenance, so we surface it as a limitation the
    # reviewer can act on (e.g. by demanding a CI-attested actor on the
    # next replay).
    ...
    for ref in actor_refs:
        ...
        # (rogue agents, mistaken commit authors, fuzzy MITM) so it
        # must flow through the same hostile-input pipeline as every
        # other backtick-wrapped ID column in this report — pipes,
        # newlines, control chars, ANSI/BIDI codepoints, overlong
        # values, and stray backticks must be neutralised before the
        # value hits the rendered Markdown bullet. ``_escape_cell_code``
        # is the helper used by the Actors / Authorities / Environment
        # tables for exactly this concern (W217); reuse it so the
        # limitations bullet inherits the same safety guarantees.
        actor_id_raw = getattr(ref, "actor_id", "?")
        actor_id = _escape_cell_code(actor_id_raw)
        # ``tier`` is also rendered inside a code-span; in practice it
        # comes from the closed ``ACTOR_TRUST_TIERS`` enumeration so it
