...
def _compute_file_debt(conn):
    ...
    # W4 (2026-05-06) — annotate every result with a roi_band so JSON
    ...
    # constant pass guarantees the bands are stable across re-runs of
    # the same index.
    if results:
        n = len(results)
        high_cutoff_idx = max(1, n // 10)
        med_cutoff_idx = max(high_cutoff_idx, n // 4)
        for i, r in enumerate(results):
            if i < high_cutoff_idx:
                r["roi_band"] = "high"
            elif i < med_cutoff_idx:
                r["roi_band"] = "medium"
            else:
                r["roi_band"] = "low"
    return results
