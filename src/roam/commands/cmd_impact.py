...
def _find_indirect_refs(conn, sym, already_affected_files: set, *, limit: int = 50) -> list[dict]:
    ...
    Pass 99 — picks up registry-dispatch consumers (e.g. cli's
...
@click.option(
    ...
    default=None,
    help=(
        "Pass 57 — bound the BFS at N hops (default: full transitive). "
...
def impact(ctx, name, hops):
    ...
    with open_db(readonly=True) as conn:
        ...
        # Pass 99 — dispatch-via-registry detection. roam's call graph
