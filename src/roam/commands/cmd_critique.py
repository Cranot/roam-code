...
def _run_batch(batch_dir: str, high_callers: int, intent_text: str | None, json_mode: bool, token_budget: int) -> None:
    """Pass 39 — review every *.diff / *.patch in ``batch_dir``."""
...
@click.option(
    ...
    default=None,
    help="Pass 39 — review every *.diff/*.patch in this directory in one pass.",
)
@click.option(
    "--high-callers",
    type=int,
...
def critique(ctx, input_path, batch_dir, high_callers, intent_text):
    ...
    # Bench-relevance hint (dogfood notes 2026-05-01, completed v12.12):
