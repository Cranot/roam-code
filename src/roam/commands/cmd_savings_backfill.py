...
SARIF is deliberately NOT emitted because transcript backfill reports
corpus-level extraction coverage, not file-located code findings. Transcript
paths are intentionally excluded from the output privacy contract.
"""
        ...
        click.echo(
            to_json(
                json_envelope(
                    ...
                    agent_contract={
                        "facts": [
                            f"{result['episodes']} historical discovery events",
                            f"{result['files_with_episodes']} transcript files",
