...
def conventions_to_sarif(violations: list[dict]) -> dict:
    ...
    rules = [
        _rule_entry(
            id=rule_id,
            short_desc="Symbol name does not match codebase naming convention",
            help_uri=_HELP_BASE + "conventions",
            default_level="note",
        )
    ]
    ...
    *changes* is expected to carry:
    ...
    rules = [
        _rule_entry(
            ...
            short_desc="Exported symbol was removed",
            help_uri=_HELP_BASE + "breaking",
            default_level="error",
        ),
        _rule_entry(
            id="breaking/signature-changed",
            short_desc="Exported symbol signature changed",
            help_uri=_HELP_BASE + "breaking",
            default_level="warning",
        ),
        _rule_entry(
            id="breaking/renamed",
            short_desc="Exported symbol was renamed",
            help_uri=_HELP_BASE + "breaking",
            default_level="warning",
        ),
    ]
    ...
    for item in changes.get("removed", []):
        ...
        results.append(
            _result_entry(
                rule_id="breaking/removed-export",
                severity="error",
                locations=locations,
                message=f"Removed exported {item.get('kind', '?')} '{item.get('name', '?')}'",
                level_mapper=lambda s: s,
            )
        )
    ...
    for item in changes.get("signature_changed", []):
        ...
        results.append(
            _result_entry(
                rule_id="breaking/signature-changed",
                severity="warning",
                locations=locations,
                message=f"Signature changed for {item.get('kind', '?')} '{item.get('name', '?')}'",
                level_mapper=lambda s: s,
            )
        )
    ...
    for item in changes.get("renamed", []):
        ...
        results.append(
            _result_entry(
                rule_id="breaking/renamed",
                severity="warning",
                locations=locations,
                message=(
                    f"Renamed {item.get('kind', '?')} '{item.get('old_name', '?')}' -> '{item.get('new_name', '?')}'"
                ),
                level_mapper=lambda s: s,
            )
        )
...
# pattern catalog at ``dev/LLM-SMELLS-PATTERN-CATALOG-2026-05-15.md``.
