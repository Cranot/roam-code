...
# ``assurance_floor`` is a computed read. Known schema/shape drift
# should fail closed at the attestation boundary; unexpected runtime
# defects should propagate so they can be fixed at the source.
_ASSURANCE_FLOOR_ERRORS = (AttributeError, TypeError, ValueError)
...
def _verified_levels(change_evidence: ChangeEvidence) -> list[str]:
    ...
    * Otherwise -> ``["SLSA_SOURCE_LEVEL_1"]``. Revision exists, that's
    ...
    Pattern-2 discipline (CLAUDE.md "lineage rule"): a known schema /
    shape error inside ``assurance_floor()`` must NOT silently downgrade
    L3 -> L1. Sibling :func:`_verification_result` returns ``"FAILED"``
    on the same exception; we mirror that by returning an empty list
    (SLSA-legal "no levels attested") so the verifier sees the same
    negative signal on both axes instead of an asymmetric silent
    fallback. Unexpected runtime defects propagate.
    ...
    except _ASSURANCE_FLOOR_ERRORS:
        return []
    if floor.get("passes"):
...
def _verification_result(change_evidence: ChangeEvidence) -> str:
    ...
    * ``risk_level`` not in ``("high", "critical")``.
    ...
    Pattern-2 discipline: known floor shape errors / missing floor /
    dangerous risk / stale evidence yields FAILED (explicit absence
    beats silent success). Unexpected runtime defects propagate.
