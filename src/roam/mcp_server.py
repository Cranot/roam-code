...
_CORE_TOOLS = {
    ...
    "roam_taint_classify",
    # v12.16 / Pass 33 — machine-readable tool catalog (1)
    "roam_catalog",
    # v12.19 / Pass 93 — agent-actionable wrappers for previously CLI-only signals (5)
    "roam_alerts",
    "roam_timeline",
    "roam_test_impact",
    "roam_disambiguate",
...
_PRESETS: dict[str, set[str]] = {
    ...
    # wedge identified by the v12.2 product agent. Everything an auditor
...
# Pass 33 — parallel registry of tool metadata so ``roam_catalog`` can
...
def _tool(name: str, description: str = "", output_schema: dict | None = None):
    ...
    def decorator(fn):
        ...
        # Pass 56 — extract richer metadata from the tool's docstring so
...
# Pass 71 — severity bucket per error code. Lets agents branch on
...
# Pass 87 — error storm rate-limit. When the same error_code fires N
...
def _structured_error(error_dict: dict) -> dict:
    ...
    Pass 28 — also fills the ``doc_link`` field so agents have a stable
    URL for self-service troubleshooting per error code.
    Pass 71 — adds a ``severity`` field (info | warning | error | fatal)
    so agents can branch on severity without parsing the message.
    Pass 87 — when the same ``error_code`` fires ≥
...
def _score_prepare_change_recipe(sub_results: list[tuple[str, dict]]) -> str:
    ...
    # High complexity + high fan-out = orchestrator (round 4 #9 dogfood:
...
async def health(
    ...
    # Pass 75 — when the issue count is huge, drop the verbose lists
...
# ---------------------------------------------------------------------------
# Pass 33 — roam_catalog: machine-readable list of all registered tools
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Pass 93 — five MCP wrappers for previously CLI-only agent signals
# ---------------------------------------------------------------------------
...
def mcp_cmd(transport, host, port, no_auto_index, list_tools, list_tools_json, compat_profile, card):
    ...
    if list_tools_json:
        ...
        for tool in sorted(tools, key=lambda t: t.name):
            ...
            # inputSchema to be present (dogfood R17 2026-05-01).
            try:
