...
# false positives like ``useKiniseisBalance`` returning ``setup`` from
# ``tests/.../useKiniseisBalance.test.ts`` (W3.1 bug fix).
_BATCH_LIKE_SQL = (
    "SELECT s.name, s.qualified_name, s.kind, f.path as file_path, "
    "s.line_start, COALESCE(gm.pagerank, 0) as pagerank "
    "FROM symbols s "
...
def batch_search(
    ...
    include_paths:
        ...
        (e.g. ``useKiniseisBalance`` matching ``setup`` from
        ``tests/composables/kiniseis/useKiniseisBalance.test.ts``).
