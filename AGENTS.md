...
a local SQLite DB. **283 commands · 244 MCP tools (17 in the default `core` preset) · 28 languages · local analysis · zero API keys.**
<!-- END auto-count:Codex-headline -->

<!-- BEGIN auto-count:Codex-authoritative -->
Authoritative counts (AST-derived, env-independent): `command_count: 283 · canonical_count: 276 · category_count: 7 · mcp tools registered: 244 · mcp tools in core preset: 17`. The `roam surface --json` envelope additionally exposes `mcp_tool_count_by_preset` for per-preset counts.
...
```
src/roam/
  cli.py              # Click CLI entry point — LazyGroup, _COMMANDS dict, _CATEGORIES. 283 command names (276 canonical + 7 aliases).
...
Run `roam --help` for the 5-verb core; `roam --help-all` for all 283 command names; `roam surface --json` for the machine-readable inventory. Use `roam --json <cmd>` for structured output.
