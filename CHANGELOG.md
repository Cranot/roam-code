...
Nothing else was renamed, no MCP tool disappeared, and no MCP preset narrowed:
`roam compatibility --json` against `dev/compatibility-baseline.json` reports
`removed=3` and that three is the whole of it. Their MCP mirrors went with them
...
- **Six more flags were accepted from the user and then silently discarded**, found by `scripts/audit_dead_cli_flags.py` and each hand-checked against `**kwargs` forwarding, `ctx.params`, `locals()` and nested-closure rebinds before being touched, because the auditor is an AST name-usage heuristic and says so. Where the flag named a real capability it was WIRED rather than removed; the three that named nothing the command could do are the removals recorded under Breaking above.
