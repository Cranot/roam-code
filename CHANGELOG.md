...
### Fixed
- **A gate that printed "nothing is proven clean" and then exited 0 — because the exit code was decided once per OUTPUT CHANNEL.** `roam ignore-drift --fail-on-found` wrote its gate twice. The `--json` copy read "any violation OR an incomplete scan"; the text copy read "any violation", and sat below an early return that the UNANALYZABLE verdict never got past. On a machine where git could not answer — cwd not a worktree, git absent from PATH, a corrupt index file, a wedged git — the text channel printed the UNANALYZABLE verdict, printed that nothing had been measured, and authorized the push. Measured on one directory: text exited 0 while `--json` exited 5. Both of this repo's wired callers read the TEXT channel, and both restate the contract the text path broke in a comment directly above the invocation; the flag's own `--help` promises to exit 5 on UNANALYZABLE. Nothing but copy-paste discipline held the invariant — `roam secrets` is the proof in the other direction, writing the same expression three times, once per channel, all three correct. The fix is a single decision site rather than two agreeing ones: `roam.exit_codes.gate_should_fail` computes one boolean before the first channel branch and every channel reads it. CLEAN and VIOLATION are unchanged in both channels; only the unmeasured verdict moves, and it moves toward refusing.
- **Three CI gates named what they could not analyse and then passed it — in BOTH channels, which is why no parity check could see it.** `roam boundary --ci` printed "no imports to analyze (corpus has 0 import edges)", `roam taint --ci` printed "22 rules loaded but not run" against an empty corpus, and `roam test-hermeticity --ci` printed "no Python test files indexed" — each exiting 0. A wrong-direction import cannot be counted in a corpus with no imports; zero findings from rules that never ran is not a security result; zero non-hermetic tests among zero tests is not hermeticity. Each now publishes `scan_incomplete: true` on that branch and refuses. That key is deliberately distinct from the `partial_success` these commands already emit, which is overloaded: `boundary` sets it for a clean run legitimately scoped to a changed range and `taint` sets it on a fully populated corpus with hundreds of findings, so gating on it would refuse every scoped run. Two of the three carried the same latent shape as the seed — a text tail whose early return skipped its own gate, and an empty-corpus branch that returned from both channels without consulting the flag at all.
- **The exit-code catalogue roam ships for CI scripts and agents described two codes no command returns.** `roam exit-codes` self-describes as a table to branch on and is exposed as an MCP tool. Measured against the program: exit 3 is returned by nothing, because the only helper that raises it is called by no command and roam auto-indexes instead of refusing; exit 4 is never returned with its documented meaning, while the guard family really does return 4 for `needs_review`. So the code that never arrives carried index advice, and the code that does arrive got the index advice it must never get — the MCP layer reported a human-judgment gate to agents as a stale index, severity warning, and RETRYABLE, and a retry re-derives the same verdict. Exit 130 (Ctrl-C) was absent from the catalogue entirely, so a caller branching over the documented range falls through every arm when a user interrupts. All three are now described as they behave; no constant was renamed and no check was deleted to make a sentence true. The test that should have caught the misreport asserted the constant's NAME rather than what a caller receives, and was green.
- **`roam doctor --help` and `roam guard-doctor` each advertised an exit code no input can produce.** doctor's help promised exit 1 for advisory-only failures; doctor has exactly two exit sites and both are exit 2, and advisory-only failures deliberately exit 0 so a fresh install does not read as broken — the code was right and the sentence was stale, in two separate docstrings plus one option help. guard-doctor promised exit 1 for an advisory failure; that rung is structurally unreachable because every failing check in the module is constructed as blocking, and non-blocking problems are emitted as warnings the exit decision does not read. A pipeline written as "if 1 warn, elif 2 fail" collapsed every advisory degradation into the success branch. The same blindness made the summary line contradict its own table — `VERDICT: healthy` printed directly above a warning row — and that is fixed; the verdict now reads `warnings`.

### Added
- **Two structural guards for the class above, both demonstrated red against the broken tree rather than asserted.** A registry-driven differential sweep runs every gate-flagged command in text, `--json` and `--sarif` across three environment classes — measurable-and-clean, measurable-with-a-real-finding, and unmeasurable-because-git-cannot-answer — and requires the same exit code from every channel, plus a refusal wherever the command's own envelope reports an incomplete scan. It enumerates from the command registry at collection time, so a newly added gate is covered without editing it, and it carries meta-guards so a discovery regex that stops matching fails loudly instead of passing vacuously. Alongside it, a static scan of every registered command module rejects the shape the seed shipped: a degradation name in the guard of a gate exit in one output-mode region while the sibling regions' gate exits ignore it. Measured at zero findings across the whole surface, so it ships with no allowlist — a future violation cannot be absorbed into an exemption nobody re-reads. The obvious stricter rule, "every channel's gate guard must be textually identical", was prototyped and rejected on measurement: it flags roughly eight commands of which seven are equivalent under normalisation.
...
- **`scripts/check_install_targets.py` — a gate that fails when a documented install target does not EXIST, and refuses rather than passing when it cannot tell.** Its sibling `sync_surface_counts.py` proves every install pin names the same version; nothing proved that version could be fetched. Three outcomes, not two: exit 0 OK, exit 1 FAIL, exit 2 UNKNOWN. Every way of not knowing returns 2 — git unusable; an EMPTY tag list (a shallow clone has none, and "no tags" is not "no releases"); a tracked file that would not open; a scanner that matched nothing; a shipped CI template carrying an UNPINNED install, whose target is whatever is latest at run time and so cannot be checked at all; a registry that did not answer; and a registry answer that is not the registry's own JSON for the version asked about. Only a clean 404 counts as evidence of absence. It is offline by default and sub-second, so it runs on the push path and in CI rather than at publish time, where a truth check cannot prevent the lie it detects.

  **Three of those refusals exist because the first version of this gate reproduced, in its own code, the defect class it was written to prevent — and each was found by attacking it rather than by running it.** (a) The empty tag list was documented as UNKNOWN in three places and implemented as FAIL: the empty set flowed into the membership test, every pin came back missing, and a plain `git clone --depth 1` was told that 13.10.0 — which demonstrably exists — "does not exist". A false assertion of ABSENCE built from missing data is the same defect as a false assertion of presence with the sign flipped. Its test asserted `!= OK` where every sibling asserted `== UNKNOWN`, and FAIL satisfies `!= OK`, so twelve green tests coexisted with a contract the code never honoured. (b) `--pypi` decided existence on `200 <= status < 300` and never read the body. An intercepting proxy answers 200, and `urllib`'s default opener honours `https_proxy` from the environment — so an empty body, a captive-portal page, malformed JSON, a redirect to another host, a 204, and a payload describing a different version all certified that an unpublished version existed. Measured against a local server, all six returned exit 0 before and exit 2 after, with a valid-JSON control still returning 0. The status is now required to be exactly 200, the final URL to still be the host asked, the content type to be JSON, the body to parse, and `info.version` to equal the version asked about. (c) A shipped template with no pin at all is invisible to a pin scanner — nothing to rewrite, nothing to check, nothing in the count — so the gate reported OK about a file it never evaluated. `templates/examples/roam-guard-pr.jenkinsfile` was exactly that: `pip install --quiet roam-code`, unpinned, the only member of a five-template family without a pin, while its README described all five as equivalent drop-ins. It is now pinned wheel-only with a post-install version assertion, and the gate refuses on any shipped template that loses its pin.

  Two further reaches were added rather than assumed. The offline check reads the LOCAL tag list, which a tag created and not yet pushed satisfies while every consumer's action ref still resolves to nothing — precisely the state the release path passes through — so `--remote` asks GitHub for the ref and `--network` runs both halves. And the report now carries its denominator: `scanned N of M tracked files` with each skip class counted, because "46 pins are fine" says nothing about the 2,709 files the sweep declined to read. A tracked file that will not open is now UNKNOWN rather than zero pins, and a file whose bytes are not valid UTF-8 is decoded with replacement instead of skipped, so an ASCII pin inside it is still seen.
...
- **`roam compile` edit-context envelopes carry a phased execution contract, and the envelope now declares the version its shape actually has.** `execution_contract_for` attaches the five-line phased tuple only when the envelope precedes an EDIT, judged by the compiler's own `_is_edit_context` signal — the same one that gates edit-probe injection — so query and read-only envelopes are not made to mandate phased edits. The 1b/4b review obligations ride an additive `orchestration_contract` block rather than arriving as sixth and seventh contract lines, for two reasons: the five-line tuple is pinned by drift guards asserting its exact length and wording, and these obligations carry policy plus criteria-template ids that a contract line cannot express, because each names a REVIEWER rather than a step. Both fields are additive and ABSENT — never empty — when not carried, so `schema_version` moves 1.1.0 -> 1.2.0 to describe what roam actually emits while leaving 1.1.0 consumers unaffected, which is why it is a minor rather than a break. Compile-time envelopes deliberately carry no artifact hashes: the agent plan and the diff do not exist yet, and a placeholder there is precisely what a later claim could pass off as a frozen value.
- **`roam compatibility` gained `--require-coverage` and `--accept-removals`, and now runs as a blocking CI job rather than a thing someone remembers to run.** A surface the baseline does not record cannot be reported as removed later, so a gap in the baseline's reach is a hole in the gate itself; `--require-coverage` exits 5 (`EXIT_GATE_FAILURE`) when the baseline does not record the whole current surface, which forces a surface change and its baseline refresh to land together instead of drifting apart. In the same spirit `--write-baseline` now refuses with exit 5 when the snapshot it is about to write would drop entries the existing baseline records, and names `--accept-removals` as the way to say so deliberately — regenerating a baseline until the diff goes away is the failure mode that flag exists to make visible. Two steps in `.github/workflows/roam-ci.yml` run it against `dev/compatibility-baseline.json`: one fails on a removed command, flag, envelope field or MCP tool, the other fails when the baseline no longer records the whole shipped surface.
- **`roam guard-pr` discloses whether its gate was actually enforced.** `exit_code` alone is 0 in two very different situations — the verdict was fine, and the verdict was blocking but this run was not gating — so the JSON envelope's `summary` now carries `gate_enforced`, `gate_suppressed` and `verdict_exit_code`. A blocking verdict that ran unenforced also writes a stderr banner naming the exact flag that suppressed it, so the human reading the terminal and the machine reading the envelope learn the same fact.
- **`canonical_procedures()` in `src/roam/plan/compiler.py`** — the canonical procedure set is published as a `frozenset[str]` instead of being inferred by consumers from whatever surface they could reach, which made the count a thing each consumer rediscovered and could get wrong independently.

### Breaking — read before upgrading

**Three CLI flags are REMOVED, and the rest of this release is behavioural.** An
earlier draft of this section said there were zero surface removals; that was
written before the removal landed and is corrected here rather than left to be
discovered on upgrade. `roam api-changes --changed`, `roam budget --staged` and
`roam compile --model-tier` no longer exist. A caller who passes one now gets a
...
**An earlier draft of this paragraph added "and three is the whole of it",
citing that same `removed=3`. That sentence is WITHDRAWN, and the reason is
worth more than the sentence was.** `roam compatibility` detects a closed enum
— removed/renamed/added commands, removed/added per-command flags, MCP
tools/params/presets — and names what it does not evaluate on every run:
`cli_categories`, `cli_command_implementation_targets`,
...
measurement, not as a closed set.** Two entries below were found only after
that correction, by diffing command behaviour rather than command surface.

**Two invocations that exited 0 at 13.10.0 now exit 2, and neither removes a
flag.** Both flags still exist, so the surface gate cannot see either.

- **`roam verify --changed FILE...` is now rejected.** `--changed` scopes
  verify to the git diff; passing explicit FILES as well was accepted and one
  of the two scopes silently won. It now raises
  `INVALID_OPTIONS: --changed scopes verify to the git diff, but explicit FILES
  were also given` and exits 2. **Remedy:** split it — `roam verify --changed`
  for the diff, or `roam verify FILE...` for those paths. A script that passed
  both was getting one of them and not being told which.
- **`roam mutate move|rename --apply --dry-run` is now rejected**
  (`--apply and --dry-run are mutually exclusive`, exit 2) where it previously
  exited 0 **and rewrote your source files** — the defect recorded under Fixed
  below. The Fixed entry states what it used to do wrongly; this one states
  what it does now, because a caller whose script passes both flags needs the
  second fact and not the first. **Remedy:** drop `--apply` to preview, drop
  `--dry-run` to write.

**One text-mode output contract moved, on a command that shipped in 13.10.0.**
`roam compile "<task shorter than the classifier's floor>"` used to print
`VERDICT: task_too_short` on **stderr**; it now prints a full
`VERDICT: facts_envelope for freeform_explore` envelope on **stdout**, carrying
`injection_advice: skip_task_too_short` where the old verdict string was. Exit
code is 0 in both. JSON mode is unchanged — `--json` still emits
`summary.verdict == "task_too_short"` under schema `roam-compile-error-v1` — so
...
Against the pre-removal source bytes taken straight from git
(`git show 2a95e2a0:<path>`, compiled rather than imported, so an editable
install cannot shadow the result), each parameter is present in `co_varnames`
and is the argval of **zero** load instructions in the command's own code object
or in any nested one: `budget`/`staged` `LOADs=[]`, `api_changes_cmd`/`changed_only`
`LOADs=[]`, `compile_`/`model_tier` `LOADs=[]`. The probe carries a positive
control, without which a clean result would prove only that the probe is blind —
`budget`/`explain`, `api_changes_cmd`/`base` and `compile_`/`artifact` are
parameters that *are* read, and all three show `LOAD_FAST`. The remaining
back-doors are closed by construction rather than by inspection: none of the
three signatures has `**kwargs` or `*args` (`co_flags` VARKEYWORDS and VARARGS
both false), none of the three parameters is a cell variable, so no nested
function can capture it (`co_cellvars=()` on all three), and
`git grep -n "ctx.params\|locals()" 2a95e2a0 -- src` exits 1 with zero matches,
so there is no generic parameter back-door anywhere in the tree. A parameter that
is never loaded cannot reach behaviour. The user-visible consequence is exactly
the error, never a different result.

**Remedies, one line each.** `api-changes` has no wider or narrower mode to
select: its only file source is `git diff --name-only <base>`, so the narrowing
`--changed` advertised was already unconditional and dropping the flag changes
nothing but the error — delete it from your command line. `budget` compares two
whole-tree metric snapshots and there is no staged-scoped metric to collect, so
`--staged` scored the entire working tree while claiming to be scoped — delete
it; the result you were getting is the result you keep. `compile` selects an
artifact on procedure and classifier confidence only, with no model-tier concept
anywhere in the plan layer; pass `--artifact` to choose an envelope shape
explicitly.

**`dev/compatibility-baseline.json` was refreshed for these three entries and for
nothing else, deliberately rather than silently.** The refresh went through the
tool's own `--accept-removals` override, which exists precisely because
regenerating a baseline until the diff disappears is the failure it is meant to
make visible. The gate was **exit 5** before — `VERDICT: breaking changes
(removed=3 renamed=0 added=0 breaking=3 coverage_gap=0)` — and the file diff is
five lines: three flag entries deleted, plus one reflowed line where `"--range"`
lost a trailing comma. Nothing else was added, nothing else was dropped, and
`--require-coverage` still exits 0, so the baseline's reach was not reduced to
make the count go down.

**The other half of the major is BEHAVIOURAL, and it is one mechanism repeated
across the fixes below: a check that could not run used to report that it had run
and found nothing.** An absent measurement resolved to a benign value instead of
to *unknown* — `()`, `[]`, `count=0`, `severity="ok"`, `score=100`,
`VERDICT: ... clean` — so a gate that never executed came back green. Six of the
fixes share a sharper variant worth stating on its own: *a value crossed a
boundary without the identity of the producer that made it, and the receiver's
default for "producer identity ABSENT" was "producer identity MATCHES" rather
than "unknown".* Every fix converts an implicit equality, or an implicit pass,
into an explicit UNKNOWN that fails closed. The consequence for you is that runs
which exited 0 can now exit non-zero on the same inputs, on the same command
line, with no flag changed.

Commands whose exit code can newly be non-zero where it was 0 **along this axis** — the two rejected flag combinations above are a different axis and are not repeated here, and this list is what targeted measurement found rather than a closed set: `roam verdict` (a required check with no recorded PASSING status no longer satisfies the requirement), `roam fitness` (a rule whose `type` no checker handles is a failure, not a silently disabled rule), `roam preflight` (a checker that raised is not a rule that passed), `roam laws check --strict`, `roam secrets --fail-on-found`, `roam py-types --ci`, `roam cga verify` (an unrunnable dirty-tree probe is UNKNOWN, and is no longer rendered as either a clean tree or as tampering), `roam guard-pr`, and `roam pr-analyze --gate --cache`. A wider set keeps its exit code but stops printing a clean verdict over a scan that did not complete — `roam adversarial`, `roam doctor`, `roam health`, `roam sbom` and `roam vuln-reach` among them; the non-degraded stdout path is byte-identical, and the disclosure goes to stderr and INTO the SARIF document rather than beside it, because a warning printed next to a SARIF file is not in the file the gate ingests. `roam cga emit` does not gain a failing exit, but it now signs a third value for the dirty-tree probe instead of `None`, so predicates emitted on a box without git are no longer indistinguishable from predicates emitted over a clean tree.

Three of these carry a real upgrade hazard, and each has a short remedy.

1. **`roam secrets --fail-on-found` now fails closed on FOUR conditions rather than one, and the one most likely to bite you is a stale index.** It still blocks on a finding. It additionally blocks on a `--severity` floor set above the highest severity the pattern table can emit — a filter under which no finding could have matched regardless of what is in the code, previously reported as "No secrets found"; on zero files having actually been read; and on any file present on disk but absent from the index. That last one is the hazard, because `ensure_index()` builds the index only when the database is ABSENT, or was left incomplete by an interrupted build, and never refreshes a stale one. **One non-ignored file added since your last `roam index` therefore turns a previously green gate into exit 5**, with `NOT PROVEN CLEAN: N indexed files scanned, but M file(s) present on disk are not in the index and were NOT scanned.` The remedy is one line: run `roam index` before the gate. The new behaviour is the correct one — the old green result was a clean bill of health covering files nothing ever opened — but it is a workflow change riding along with a bug fix, not just a bug fix. CI checkouts that build the index in-process on every run are unaffected, because there is no stale index there to be stale.
2. **`roam laws check --strict` now decodes UTF-16, so a `--diff-file` that previously scored "0 violations" may newly report REAL violations on an unchanged command line.** PowerShell 5.1 writes UTF-16LE by default, so `git diff > x.patch` on Windows produced a file that the previous `read_text(encoding="utf-8", errors="replace")` turned into a BOM replacement character followed by the real text interleaved with NULs. Measured on the pre-fix code, same diff, both encodings: UTF-8 gave 1 file parsed and 6 added lines; UTF-16 gave 0 files parsed and 0 added lines — with `error=None` in both cases. Every line of the mangled text classified as `other`, so no file was ever opened and no law saw a single added symbol, and the run ended `VERDICT: 0 violations` and exit 0 from a strict gate over a diff it never read. The existing fail-closed guard did not catch it because it keys on `not diff_text.strip()`, and text full of U+FFFD and NULs is not whitespace. The reading is now the repo's hardened `decode_views` set (BOM dispatch, NUL-stripped, byte-transparent), so a genuine UTF-16 diff is evaluated correctly rather than refused — which is the better outcome and also means violations that were always present in your diff can surface for the first time on an upgrade. Bytes that yield no diff structure under any reading are returned as an ERROR into the existing "DIFF UNAVAILABLE — no laws were evaluated" path, which sets `partial_success` and exits `EXIT_GATE_FAILURE` under `--strict`; that also covers a wrong file passed to `--diff-file`, a truncated download, and a context-format diff the parser does not speak. An empty or whitespace-only file is still NOT an error, because an empty diff is a real, well-formed answer meaning "no changes".
3. **`roam pr-analyze --gate --cache` will recompute verdicts it would previously have served from cache, and your first run after upgrading is a cold cache by construction.** The cache key hashed diff text, the rules file, the block threshold, the language override and a `CACHE_VERSION` whose own comment scopes it to "bump when the envelope shape changes" — so every term was an input or a serialization tag, and nothing named the producer. The cached value is `summary.verdict`, which `_serve_from_cache` turns into `sys.exit(EXIT_GATE_BLOCK)`. Measured with one set of inputs and a fresh process either side of a change to the module that derives the verdict, the key was byte-identical before and after, and a bundle written by the old logic was served under the new logic's key as `{'cache_hit': True, 'summary': {'verdict': 'SAFE'}}` — so the gate could pass or fail a PR on a verdict the running code never computed. The key now carries an analyzer fingerprint: mtimes of every module in `roam.commands.pr_analyze` and `cmd_pr_analyze`, discovered via `pkgutil` so a module added later needs no maintainer discipline, plus the installed-version stamp. Neither term alone is sufficient. The practical effect is that a PR passing on a cached `SAFE` may block the first time the current code actually evaluates it.

### Fixed
- **`roam doctor` measured that a file existed and published the cause "a git process was killed mid-write".** `git repo health` reported `.git/index.lock` as stale on presence alone. git holds that lock for the duration of every normal index write, so a lock created moments ago is a LIVE writer — the one state the message rules out. Presence was measured; death was inferred. The check's own comment said it: *"the lock's presence is the finding; its age is only flavour."* Age is not flavour — it is the entire difference between a writer and a corpse. Because the check is BLOCKING, the inference had teeth: under pytest-xdist `roam doctor` exited 2 on a healthy repository, taking one CI lane red at a commit the other three passed, twice in one day, and a false positive about a healthy repo blocked this release. Now three-way, and only the middle case changed: younger than the threshold is a live writer and is not reported; older is stale and reports exactly as before; an age that cannot be read still reports, because a failed `stat` cannot prove the lock is live. The threshold is generous on purpose — an abandoned lock is permanent, so a late report costs nothing, while a live lock called stale costs a failed run every time any tooling touches git in parallel. The cheaper fix was rejected: widening the test's accepted exit codes would have hidden this from CI while leaving it live for every user, and blinded the same assertion to a real blocking failure later. The test file's own docstring already said *"every positive case has a matching 'must not fire' pair"* — this check was the one shipped without its pair, which is why it shipped wrong. It has one now. **Not closed:** the unreadable-age branch has no test; driving it needs `Path.stat` to fail for one path, but `Path.exists()` is implemented via `stat`, so any patch reaching the branch defeats the guard before it.
- **Two CVEs landed against a pinned CI tool between one push and the next.** `pypdf` 6.14.2 acquired CVE-2026-71852 and CVE-2026-71870 on 2026-08-07, both fixed in 6.15.0. Nothing here changed to cause it — one commit passed the dependency-audit lane and the next failed it with only changelog files in between; the advisories were published in the gap. The gate behaved correctly and is not the defect: `pip-audit --strict` fails on ANY known vulnerability in the locked graph, which is the point of auditing a locked graph rather than trusting it. All four Python lanes failed identically, which is how this was told apart from the single-lane index.lock race above. `pypdf` is CI-only, so no shipped artifact carried the vulnerable version. It is bumped rather than waived, for the reason the `pillow` pin already records: dev-only code is still installed in CI and must audit clean. The `ci`-group drift guard then caught its own group changing — it pins the exact literal in two places so a version cannot move in `pyproject.toml` alone, and it refused a change that touched one file and not the other.
- **The Jenkins example told readers to commit the secret that authorises their builds.** `templates/examples/roam-guard-pr.jenkinsfile` set the Generic Webhook Trigger's token as a literal inside its `GenericTrigger` block. In that plugin the token is not an identifier — it is the shared secret in the invocation URL (`/generic-webhook-trigger/invoke?token=…`), so anyone able to read the file could fire builds on any Jenkins that used the template as written, and being used as written is what a template is for. It now reads `tokenCredentialId: 'roam-guard-webhook-token'`, the plugin's own indirection, with the credential stated as a requirement in the file header. Deleting the setting was not an option and the comment at the call site says so: a `GenericTrigger` carrying neither `token` nor `tokenCredentialId` accepts UNAUTHENTICATED invocations, which is worse than a published one. **This repository's own pre-push secret scan is what caught it** — it had been in the file since it was written, and surfaced only when an unrelated commit rewrote the file and made the blob new in a push range. It was not suppressed with the scanner's `# secretsallow` marker: the finding was true, and silencing a true positive to make a push succeed is the failure these gates exist to prevent. Fixing it at the tip was also not enough — the scan reads pushed HISTORY, so the literal had to leave every commit in the range, which is why the unpushed commits were rebased. No wheel-shipped template was affected: `src/roam/templates/ci/Jenkinsfile` has no triggers block, measured rather than assumed. What this does not close: no test asserts the absence of inline trigger tokens across the example family, so a future template can reintroduce one and only the secret scan will object, and only once that blob changes.
- **The version pins on public `main` told every reader and every machine to install a release that did not exist, and the gate that runs on every push is what wrote them.** `scripts/sync_surface_counts.py` held one belief about version literals: that each must equal `pyproject.toml -> version`. That is true of literals that DESCRIBE this artifact and false of literals that INSTRUCT a fetch, and the two were one class. So the moment 14.0.0 was declared, the sweep rewrote ~50 install instructions to name it and reported "All release-version pins match pyproject 14.0.0" — while the pip pin returned **HTTP 404** from PyPI and the action ref resolved to no ref at all. The worst instances shipped INTO A USER'S REPOSITORY: `roam ci-setup --write` and `roam init --with-ci=github` emit files the user commits, and those files carried a pip pin and an action ref that both fail at the user's CI runtime. Reproduced end to end before and after in a scratch repository. The class is now split — identity literals (`CITATION.cff`, `codemeta.json`, the plugin manifest, `server.json`'s own version, generated doc headers) still track pyproject; install literals (the pip pins, the action refs, the composite action's `version` input and default, `server.json`'s `packages[].version`, the roam-guard post-install equality assertion) track the last PUBLISHED release, i.e. the highest `v*` tag. `_published_version()` refuses when the tag list is empty or unreachable rather than falling back to the declared version, because that fallback is the defect. `.github/workflows/roam.yml` loses its exemption: it was the one file that got this right, and it is now correct by construction rather than by exception.
- **A light reindex left every edited symbol invisible to lexical search — 11.4% of this corpus, with no error, no warning and no degraded-mode disclosure.** `Indexer.run(light=True)`, the path `roam verify` takes after every edit, skipped phase 7 entirely. But phase 1 CASCADE-deletes an edited file's symbols and re-inserts them under fresh AUTOINCREMENT ids, so skipping the phase left the new ids with no `symbol_fts` row while the old rowids lingered as orphans holding pre-edit text. `_FTS5_SEARCH_SQL` inner-joins `symbols ON sf.rowid = s.id`, so an orphan row is dropped and a symbol without a row can never be returned. Measured on the roam-code index: 5,174 symbols missing from FTS and 4,012 orphan rows, worth roughly 13 points of recall@20. Row COUNTS cannot detect this — an in-place edit keeps the count identical — which is why the check is on the id sets rather than on the totals.
- **`roam init` printed "Roam is ready: N files, 0 symbols, 0 edges" and exited 0 for six structurally different situations, four of them broken.** An absent measurement rendered as a definite success value, in the first command a new user runs. It had already produced one wrong conclusion in this estate: a measurement run against a venv that had indexed 0 rows got green results from every indexing-dependent assertion. Init now distinguishes the legitimate empty cases (empty repo, docs-only repo, everything excluded by `.roamignore` or `.gitignore` — each now NAMED) from the broken ones. The refinement matters as much as the refusal: "files in a supported language and no symbols" is also true of an empty `__init__.py` beside an import-only module, which parses perfectly and declares nothing, so the refusal keys on a corpus that understood nothing rather than on a corpus that found nothing.
- **A per-process parse-error counter made one bad file refuse every later index in the same process.** `parse_errors` was module state nothing ever reset, harmless while it only fed a log line — until the corpus verdict started reading it to decide an exit code. Index a file with a syntax error and every subsequent index in that process inherited the count and was refused as a broken corpus. A one-shot CLI never sees it; in-process callers do, and it cost 358 errors plus 32 failures across the empty-corpus suites, all as setup failures. The guard around `tree.root_node.has_error` for older tree-sitter bindings was fixed in the same area for the same reason: swallowing that `AttributeError` made every file look clean and downgraded a refusal into a false success, on the counter that decides whether a zero-symbol index is an empty tree or a broken one.
- **An extraction changed which count gates the vector rebuild.** Pulling the rowid diff out of `build_fts_index` into `_sync_fts_rowids` returned rows-INSERTED and branched the TF-IDF/ONNX rebuild on that; the original branched on the FRESH ID SET. The two differ whenever a symbol lacks a surviving `files` row, and the branches pass different labels and different `exc_types` to `_build_vector_signals` — so the divergence silently changed which ONNX exceptions get logged rather than propagated. The sync now returns `(deleted, fresh_count, inserted)` and gates on `fresh_count`, restoring the pre-refactor branch exactly while callers reporting work done still get the truthful inserted count.
- **`--budget 0` promised unlimited in three help strings and was the value that capped.** click `default=0` plus `_apply_envelope_budget` honouring only `budget > 0` meant an explicitly typed 0 fell through to the default cap. Measured: `roam --budget 0 taint --json` returned `truncated: true`, `budget_tokens: 20000`, `summary.findings: 894` and ten findings in the payload.
- **`roam mutate move` / `roam mutate rename` rewrote source files under `--apply --dry-run`.** Both declared `--dry-run`, bound it into the signature, and never read it; the write decision was recomputed from `--apply` alone. With no flags the default is already a preview, so nothing ever looked wrong — which is why it survived. An explicit preview that writes is the one direction of this defect that costs the user their working tree.
- **Six more flags were accepted from the user and then silently discarded**, found by `scripts/audit_dead_cli_flags.py` and each hand-checked against `**kwargs` forwarding, `ctx.params`, `locals()` and nested-closure rebinds before being touched, because the auditor is an AST name-usage heuristic and says so. The dispositions were not two but four, and an earlier draft of this line named only two — which concealed the one that can break a caller's script. One was WIRED (`roam file --full`); three named nothing the command could do and were REMOVED (the entries under Breaking above); one, `roam init --yes`, is KEPT as a documented no-op; and one, `roam verify --changed` combined with explicit FILES, became a REJECTION of a previously-accepted invocation and is recorded under Breaking above with its remedy. A flag that is neither wired nor removed is the disposition a "wired or removed" sentence cannot express, so it silently fell out of the account.
- **`roam taint` published intra-function co-occurrence as proven source-to-sink dataflow**, at `confidence: high`, `risk_score: 100`, and a fabricated three-hop path — 891 of 894 findings on this repo carried `path_length: 3` for a path nobody walked. The JSON envelope was made honest first; the SARIF projection of the same finding is corrected under its own entry below, because SARIF is the surface a Code Scanning gate actually ingests.
- **The RELEASE-tier linkcheck gate could not fail.** `scripts/linkcheck.py` gates its exit code on `--strict`; without it the script prints every dead link and anchor it found and still exits 0. The tier registered it flagless and `GateRunner._run` records PASS on returncode 0, so the gate reported green on a broken landing page — while CI ran the `--strict` form. A tier that advertises itself as "what CI verifies" and runs a weaker form of the same command is worse than one that does not run it at all.
- **The pre-push hook ran whatever `python` PATH resolved to, not the project's interpreter.** Measured on a dev box: PATH-`python` was a global 3.14 and the gate failed a correct push over 7 test errors in dependencies that do not exist under the project's 3.12 venv. A gate that blocks on an environment nothing ships teaches people to reach for `--no-verify`, which is the one habit the hook exists to make unnecessary.
- **`roam verify` on a clean tree printed `VERDICT: PASS (score 100/100)` and `OK — all checks passed` having run zero checks.** `checks_run` was `[]` and `files_checked` was 0 on that branch. The human-facing lines are corrected here; the agent-facing `agent_contract` block — the one the MCP tool returns verbatim — is corrected under its own entry below, because fixing the text and leaving the contract would have repaired the surface nobody's automation reads.
- **`is_test_file` matched its test-directory list with a bare substring, and the anchored replacement was wrong in BOTH directions.** The first fix replaced an unanchored `in` over the whole lowercased path with an exact / delimiter / camelCase component match; two independent adversarial reviews then refuted it on measured real files. Scored against a 20-row refutation corpus: legacy substring 13/20, the anchored replacement **0/20** — worse than what it replaced — and the shipped form 18/20, with the two remaining misses named in the source rather than left implied. Recorded as a pair because the intermediate state is the lesson: an anchoring that is obviously more correct than a substring can still be strictly worse than it, and only a refutation corpus says which.
- **Three distribution manifests were read by three different publication channels and swept for drift by none of them.** `.claude-plugin/plugin.json` sat four releases behind on a field the plugin host treats as the UPDATE TRIGGER, so every installed copy was frozen on an old manifest, `mcpServers` block included. It was not forgotten once, it was unreachable: the value carries no pin shape, so the shape-anchored sweep could not see it, and it was in no count registry either. It and the two citation manifests are now covered structurally. Separately, a release bumped the machine-read version fields and left five human-readable prose captions quoting the prior release — two of which are consumed by downstream tooling rather than only read by humans.
- **A published benchmark number that no reproduction confirms is not a result.** `bench/retrieve/SUBMISSION.md` is public, opens with "Quick reproduce", and told the reader to run a command; three different recall sets existed in the repo and nothing re-derived any of them. All of it was re-measured on a named commit with a fresh full index and the numbers replaced with what that run produced.
- **The surface-compatibility gate counted MCP presets and never read their MEMBERSHIP.** `_COVERED_DIMENSIONS` listed `mcp_preset_counts` and `_UNCOVERED_DIMENSIONS` did not list membership, so membership sat in neither — the harder shape to notice, because a number WAS recorded and diffed and the claim over "presets" read as complete. A count is a lossy projection of a set. Reproduced pre-fix by swapping one real member of the core preset for another real tool at an unchanged count of 17: the snapshots were byte-identical.
- **The guard on the pre-push RELEASE note could not see a gate whose result decides control flow.** `_unconditional_gate_labels` filtered candidate statements by KIND — only `ast.Expr` — so a `runner.<gate>(...)` call in value position was never a candidate, which is house style in that very file. Widened to read the value position too. The residual reach of that extractor is now stated where it is claimed rather than implied; see the entry below.
- **Two release guards asserted a frozen version literal and so failed a correct release for no defect.** A hardcoded number in a pin guard cannot distinguish the release it guards from the bug it catches: it agrees with a template that never got bumped (shipping the previous wheel to every consumer, green) and disagrees with one that was bumped correctly (red, no defect). The direction of the signal is inverted precisely when it matters. Both now derive their expectation — and, since the pin class split above, they derive it from the LAST PUBLISHED release rather than from the declared one, which is a different number for the whole bump-to-release window.
- **The surface-compatibility gate printed `surface_coverage: "complete"` and `0 entries outside baseline coverage` over a dimension it could not read at all, and the commit directly above this one in the changelog is the proof.** `dev/compatibility-baseline.json` recorded `mcp_tools` as a flat list of 244 names with no parameters, so a removed MCP tool *parameter* was outside the gate's reach by construction. The three-flag removal recorded above also removed the two MCP wrapper mirrors of those flags — `staged` from `roam_budget_check`, `model_tier` from `roam_compile` — in the same commit. The CLI half went **exit 5** and forced a deliberate roll-forward; the MCP half passed the same gate in the same run. Snapshots built from the two revisions were **byte-identical** at 5,833 bytes, and running the shipped gate across that pair with `--ci --require-coverage` gave `VERDICT: no regressions (removed=0 renamed=0 added=0 breaking=0 coverage_gap=0)`, **exit 0**. Two parameters vanished and the artifact could not tell the two builds apart.
  **That is not an unmeasured claim, it is an unfalsifiable one, which is why the fix had to be two-sided.** `coverage_gap` sums additions across the dimensions the snapshot collects, so a dimension the snapshot never collects contributes zero no matter what happened to it — the completeness assertion could not have gone red. So the measurement was **widened**: the snapshot is now schema **1.1.0** and records `mcp_tools` as `{tool: [parameter, ...]}` (244 tools, **936 parameters**, +17,591 bytes on a 69,235-byte artifact, +25.4%), `removed_mcp_tool_params` is the sixth term of the breaking tally, and the same real pair now reports `VERDICT: breaking changes (removed=2 ... breaking=2)` naming `roam_budget_check(staged)` and `roam_compile(model_tier)`, **exit 5**. The parameter diff lives inside `_diff` rather than in a wrapper, so `--write-baseline`'s ratchet inherits parameter-erasure refusal exactly the way it inherits graceful-rename handling.
  **And the claim that remained was narrowed to what it measures.** Even with parameters recorded, `complete` is a statement about recorded dimensions, so the envelope now publishes `covered_dimensions` (6) and `uncovered_dimensions` (5) beside it, `surface_coverage_scope: "covered_dimensions"`, and both lists in `agent_contract.facts` — and the text output prints them on **every** run including a clean one, because a clean run is exactly when "no regressions" gets read as "nothing regressed". Each covered dimension is asserted to move the verdict when perturbed and `cli_categories` is asserted *not* to, so neither list is prose. A pre-1.1.0 baseline is treated as **unrecorded** rather than as "no parameters": it counts 244 into `coverage_gap`, flips `surface_coverage` to `partial`, and fails `--require-coverage` with exit 5. Reading the absent key as an empty one would have reproduced this defect one schema version later.
- **`roam --json verify --changed` on a clean tree told an agent `PASS`, `score 100` — having run zero checks — in the one block the MCP tool returns verbatim.** The human-facing lines were made honest earlier; the agent-facing `agent_contract` was not, and it is the block that actually reaches the primary consumer, because the MCP tool `roam_verify` shells this exact command and returns the envelope with no post-processing of `agent_contract`. Measured on a clean tree before the fix: `facts` was `["PASS", "score 100", "70 threshold findings", "0 files checked", "violation count 0"]` and `risks` was `[]`, beside `summary.checks_run == []`, `summary.files_checked == 0` and `summary.state == "no_changes"`. An agent that reads the bounded contract block — which is what the block exists for, and what a context-tight agent is told to read instead of the full payload — came away with a verdict and a quality score for work nobody did, and an empty risk list that reads as "no concerns". It now reads `NOT VERIFIED: 0 of 35 checks ran` / `0 files checked` / `state no_changes: no file was read and no check ran` / `threshold 70 was never applied to any file` / `score 100 is the not-run default, not a measured result`, with two risks naming the same thing. The check count is derived from `_ALL_CHECKS` rather than written down, so registering a check cannot turn the fact into a false count.
  **The wire numbers deliberately did not move, and `summary` gained nothing.** `compile verify` shells this command and validates the result against a pinned no-changes contract that requires `summary.score == 100` and `categories[*].score == 100`; separately, its `_require_known_shape(summary, allowed=_VERIFY_SUMMARY_KEYS)` is a CLOSED key set, so an added summary key would raise `summary_schema` and exit `EXIT_TOOLCHAIN` on **every** clean tree. The disclosure therefore lands in `agent_contract`, which that boundary only requires to be a dict. The nine-key summary set is now pinned by a test, so a later "let's just add a field" cannot break the boundary quietly, and the four machine-readable not-run discriminators (`state`, `files_checked`, `checks_run`, `violation_count`) are unchanged.
- **`{"threshold": N}` rendered as `"N threshold findings"` in every agent-facing fact that carried it — a gate setting read as a count of findings.** The fact humaniser classifies a summary key as either a measurement name (`"health score 75"`) or a count noun (`"5 critical findings"`); `threshold` fell through to the count-noun rule, so `roam verify`'s `threshold: 70` became the fact `70 threshold findings`, which states that seventy things were found. Measured blast radius: 40 `"threshold":` summary entries across 14 command modules (`cmd_check_rules`, `cmd_compiler_health`, `cmd_complexity`, `cmd_context`, `cmd_diff`, `cmd_docs_coverage`, `cmd_drift`, `cmd_envelope_diff`, `cmd_fitness`, `cmd_health`, `cmd_over_fetch`, `cmd_smells`, `cmd_verify`, `cmd_watch`), all emitting the same garble. `threshold` is now a measurement-naming suffix and the fact reads `threshold 70`.
  **That list had three copies and a guard covering only two of them.** The humaniser's `measurement_suffixes`, `agent_opt._MEASUREMENT_SUFFIXES` (the runtime weak-fact detector) and the CI lint's hand-maintained mirror must agree, or the lint rejects the string the humaniser just produced. Parity between the latter two was already pinned; the copy that decides how a fact is actually rendered was not covered by anything, which is how a suffix added to two of three would have gone unnoticed. A new test AST-reads the humaniser's list out of the source — deliberately not importing it, since it is a function local and since two names for one object is not a parity check — and asserts set equality with the lint's, naming which side each drifted entry is on.
- **`roam taint --sarif` published the claim `roam taint --json` disclaims, on the same index and the same finding.** The JSON envelope is honest about what the analyser did: all 637 findings on this repo carry `confidence: low`, `reason: "source and sink co-occur in the same enclosing function; no dataflow path was computed"`, `evidence: "co_occurrence"`, `path_length: null`, and a `risk_score` of 0 that excludes every one of them. The SARIF projection of that same finding came out as `level: "error"`, message `Tainted flow: environ → run`, and a three-step `codeFlows` threadFlow — and a codeFlow *is* the assertion that those hops were walked. They were not: `cmd_taint` builds `path` from `f.path_symbols` for every finding, so a co-occurrence entry carries a synthesised source/enclosing-symbol/sink triple, and that triple is what became the threadFlow. SARIF is the machine-read surface a Code Scanning gate consumes, so it is where a fabricated flow does the most damage — 637 co-occurrence hits presented as 637 proved injections, at the one severity that fails a build.
  **A finding whose `evidence` is present and not `dataflow` is now published without a `codeFlows` entry, never at `level: "error"`, with a message that names the basis (`Possible tainted flow (co-occurrence only; no dataflow path was computed): environ → run`), and with `properties.evidence` / `evidence_detail` / `path_length` so a triage user can see what it rests on.** Measured before and after on the same index: `level` counts went from 637 `error` to 633 `warning` + 4 `note`, `codeFlows` present went 637 → 0, result count unchanged at 637, document still parses and still reports `version 2.1.0`.
  **The downgrade is conditional, and the condition is `evidence` — not `confidence`.** `confidence` and `reason` are attached downstream by `wrap_findings` and are absent from the `findings_dump` dicts the emitter receives, so keying off them would have been a no-op. A genuine `evidence: dataflow` finding keeps its level, its `Tainted flow:` message and its codeFlow byte-for-byte; this repo produces **zero** dataflow findings today, so that regression would have been invisible without the synthetic case now pinned in `tests/test_sarif_flag.py`. An **absent** `evidence` key is treated as unknown and changes nothing — `cmd_taint` stamps the field on every entry it builds, so every finding roam actually emits takes an informed branch, and manufacturing a disclaimer out of a missing field would be the same fabrication pointed the other way. That choice is pinned by its own test rather than left to inference.
- **The co-occurrence disclosure was deleted by the warning that came after it.** `cmd_taint` appends `"N of M finding(s) rest on co-occurrence only …"` to `summary.warnings_out`, then the `qualified_only` rules-lint branch assigned the same key with a bare `=` instead of appending — so any run producing both signals shipped the lint warning and silently dropped the sentence that qualifies every finding in the envelope. Every neighbouring branch in that block (zero-anchor rules, W607-AY/CJ substrate markers) already reads-then-appends, which is what makes this an omission rather than a convention. On this repo `rules_lint.qualified_only_violations` is 0, so the branch never fired and the live disclosure survived; the defect is reachability, and `tests/test_r3_taint_cooccurrence_disclosure.py` forces both signals into one run to hold it. A second test pins the substrate markers in their terminal position so the change cannot reorder them.
- **The pre-push gate disclaimed a gate it had just run.** On success the RELEASE tier of `scripts/prepush_check.py` prints a list of CI lanes that "are NOT run by any tier and stay unproven here", so an operator does not read local green as "CI will be green". One entry in that list was false: it named `roam ignore-drift --fail-on-found`, which `_run_leak_gate` registers and which `main()` invokes unconditionally **before** any tier branching — so it runs in FAST, FULL and RELEASE alike. Measured on a live FAST run: `[prepush] roam ignore-drift --fail-on-found: PASS (0.4s)`. A note that disclaims proven work is worse than no note, because the reader's only correct response to "unproven" is to spend a CI round proving it. The bullet is narrowed to `roam secrets --fail-on-found`, which is the only one of `secret-scan.yml`'s three steps genuinely absent from the push path — the other two, the repo leak-catalogue scan and this tracked-but-ignored gate, both run in every tier. **The gate itself is untouched;** making the sentence true by deleting the check would have been the same defect pointed the other way. A regression test pins THIS instance, and the class it did not close is closed by the entry directly below.
  **`AGENTS.md` carried the same claim and is corrected to match.** Its release-discipline section listed `secret-scan` among the lanes the release tier leaves uncovered; it now names the single step that is uncovered and says the other two run in every tier.
  **The guard that was built to prove that note honest could read 7 of the 8 gates a live run records, and the note is now DERIVED rather than guarded.** The first version AST-walked `main()` for `runner.<gate>()` calls, collected the labels those methods pass to `self._run(...)`, and asserted no label appeared in a bullet. It was widened twice and it was still bounded by syntax: a keyword-argument call, an aliased receiver, a starred argv, a non-literal label, a module-level helper and an inline `results.append(...)` all evaded it, six shapes found in one probing session with no terminating condition on the search. Two of those shapes were LIVE in the tree — `run_pytest_bundle` builds its label with an f-string, which is why 8 gates ran and 7 could be named, and `run_release_temp_capacity_gate` appends its result inline. The shape that settles it is not a syntax at all: a correctly-written positional literal placed under `if release:` is skipped as "not unconditional" and passes, while the note it certifies is printed ONLY in the RELEASE tier. No widening of a reader reaches that, because the defect is in the predicate rather than in the parsing. So the disclaimed-lane list became data (`_RELEASE_UNPROVEN_LANES`) and `_print_summary` filters it against `runner.results` — the list it already receives as its first parameter — printing `(no longer unproven: … — ran as …)` rather than silently shortening itself, because a suppression with no stated cause reads as an edit rather than a measurement. That makes the class impossible by construction instead of detected after the fact. All eight shapes above, including the tier-placement one, were confirmed RED against the pre-derivation script and green after. **What it does NOT close, stated because narrowing a claim twice is not a fix:** a bullet is still related to a gate by SUBSTRING, one hand-maintained probe per bullet; it closes only the FALSE-disclaimer direction, so a CI lane added to `.github/workflows` and never disclaimed at all remains invisible to any runtime record and would need a different guard over workflow YAML; and a red gate truncates the record, which is correct for a note describing what THIS run proved but means the note must be computed in-process at print time. `AGENTS.md` held a fourth hand-maintained copy of the lane list and now points at the note instead of restating it.
- **The guard built to prove that note honest could not read the wiring shape `main()` already uses, so the note could have gone false again with the guard green.** `_unconditional_gate_labels` learned a gate's label only by resolving `runner.<method>()` to a `FunctionDef` and walking THAT function for `self._run("<label>", ...)`. A gate wired as `runner._run("<label>", ...)` directly in `main()` resolves `method='_run'`, walks `GateRunner._run`, finds no nested `self._run` inside it, and contributes **nothing**. Measured on a copy of the script carrying one extra unconditional `runner._run("roam compatibility --ci", ...)` — a label the note's own bullet list already disclaims: the guard read the same **7** labels as the real tree and passed. The extractor now reads the direct form too, and on that same mutated tree reads **8** and fails with `'roam compatibility --ci' is registered by main() but disclaimed in the bullet 'roam compatibility --ci / --require-coverage'`. On the real tree it reads the same 7 labels as before and stays green — a strict widening of the measurement, not a new finding.
  **The evading shape is the next move the note itself anticipates, and the widening is bounded so it cannot over-fire.** `roam compatibility --ci / --require-coverage` is a live bullet in that "NOT run by any tier" list, and wiring that lane into the push path is the obvious follow-up to the compatibility work above. A negative control pins the other direction: the identical `runner._run(...)` nested under `if release:` is **not** collected — 7 labels, green — because those lanes genuinely do not run in FAST and the note is right to disclaim them. All **5** `runner._run(...)` calls in `main()` today are inside `if release:` and **0** are direct children of its body, so the widening changes nothing about the current tree; it closes the shape before something occupies it.
  **The extractor's own blindness tripwire did not fire on the shape that blinded it, so the tripwire moved to the output.** `assert unconditional_methods` was written for "the AST shape this guard reads has changed and the guard is now blind" — but `runner._run(...)` IS a `runner.<method>()` call, so on a `main()` whose always-on gates are all direct-form the assertion sees `['_run']`, passes, and the extractor still returns zero labels. The verdict is `labels ∩ bullets == ∅`, which is vacuously true on an empty set, so the guard then certifies the note with maximum confidence having read nothing at all. Measured on such a tree: zero labels, tripwire silent, verdict green. The assertion is now on the labels actually read, so an extractor that reads nothing fails loudly instead of certifying silence — including for a third wiring shape neither branch anticipates. Both properties are pinned by tests over a fixture carrying all three wirings (grouped, direct, tier-conditional), and both were confirmed RED against the pre-widening extractor and green after.
- **The guard on the pre-push script was not run by the pre-push script, while its own docstring said it was.** `tests/test_prepush_gate_wired.py` opens with "This module pins the contract via pure AST inspection (no subprocess, no index build) **so it runs in the FAST bundle itself**". Measured: a `--collect-only` over `FAST_PYTEST_GUARDS` returned **0** node ids from that file, and it was absent from `FULL_PYTEST_GUARDS` too. So every contract it pins — the bundle membership list, the once-and-only-once whole-tree leak scan, and the RELEASE note not disclaiming a gate that runs in every tier — was enforced by CI *after* the push and by nothing before it, while the docstring read as local coverage. The widening directly above is worth nothing on the push path if the guard carrying it does not run there.
  **The claim is made true rather than deleted, and the file now pins its own membership.** `test_prepush_gate_wired.py` joins `FAST_PYTEST_GUARDS`, and it is added to this file's own `_EXPECTED_FAST_GUARDS` set, so dropping it out of the bundle fails the guard instead of quietly restoring the gap. The same `--collect-only` now returns **10** node ids. Measured cost, same box and same 4-worker pool: FAST pytest bundle **58.1s → 59.1s**, whole FAST tier **94.1s → 94.6s** — one second for the file it guards, which is exactly the file whose change should run it.
- **`CONTRIBUTING.md` documented a version-sync guarantee the repo did not provide, and the next release would have shipped an action that installs the previous release.** The claim was that `pyproject.toml` is the single source of truth and "everything else syncs from it via `scripts/sync_surface_counts.py`". That script synchronises surface *counts*; it contained no version handling at all, so the version half of the sentence had never been true. The consequence is not cosmetic: `action.yml`'s `version` input default and `server.json`'s PyPI package pin are the fields that decide what a downstream consumer actually installs, so a release that bumped `pyproject.toml` alone would publish a composite action whose default installs the version *before* the one just released, and tell the MCP registry to fetch the same stale wheel. The stale claim survived for the same reason the underlying gap did — nothing ever failed on it. A survey of the tracked tree found 52 files carrying the release literal, not the 15 the section implied, including the entire shipped-CI-template family and four copy-paste guard examples. Crucially they are **not one kind of thing**, and a find-replace across them would have been a worse defect than the staleness: recorded benchmark measurements and a dated sample audit deliverable name the version they were *produced by*, so re-stamping them falsifies a result; the repo's own workflow pins its own published action and therefore cannot point at a tag that does not exist yet; and the compile-cache modules' version literals turned out to be narrative in comments (proven with the tokenizer, and now pinned as such) because the runtime stamp is read from the installed `dist-info` directory name — bumping a literal there would have been a cache-key behaviour change wearing a version sync's clothes. The fix extends the script CI already runs rather than adding a new one, so the documented sentence becomes true instead of being deleted. Pin detection is **shape-anchored, not value-anchored** — it matches the *form* of an install pin rather than the current literal — so a new surface is covered the day it lands rather than the day someone remembers to register it, and prose that merely names a past version is untouched by construction. Sites with no reusable shape (an action input default, a registry package pin, generated doc headers, landing-page version stamps that the existing landing-page sweep did not recognise) are anchored individually on surrounding structure so they cannot match anything else. Everything excluded is excluded by an explicit registry that must carry a reason, and a stale entry naming a file that no longer exists is itself a failure. Two assertions in the composite-action security suite that compared against a hardcoded literal now compare against `pyproject.toml`, turning them from restatements of the mistake into the invariant they were meant to express. Regression suite in `tests/test_w1501_release_version_pins.py`, built around controls in both directions: the negative control proves the gate reports each derived shape when it disagrees and that `--write` converges, and the false-positive controls — a CircleCI config schema version, a pinned third-party action version, changelog prose naming a shipped release, a recorded benchmark engine version, and a PEP 440 grammar example inside an error message — prove the sweep leaves non-pins alone, which is what makes it safe to run with `--write` during a release. Verified against a clean checkout rather than the working tree: the gate exits 0 on the true tree, exits 1 naming file and line when one derived occurrence is reverted, and exits 1 across every derived surface when the version is bumped with nothing else touched.
- **Six more places where a check that could not run reported that it had run and found nothing** — found by a detector built from our own fixed defects rather than from intuition. `verify-imports` returned a zero-result SARIF document for a file it could not read, so a stale index turned every hidden unresolved import into a green gate on a scan that never happened; the projection gains an `unverifiable-file` rule (`defaultLevel: warning`) so the artifact a CI system ingests says the file was skipped rather than saying nothing. `dead-public-surface` treated an unreadable package manifest as "no public entry points", which is indistinguishable from a package that genuinely exports nothing — the difference between "this symbol is dead" and "I could not tell", published under the same word; unresolvable status now carries an `unverifiable` prefix that consumers must treat as UNKNOWN, never as proof of publicness and never as proof of deadness. `test-hermeticity` counted a file it failed to parse as examined, `suppress` treated an unreadable suppression store as an empty one (so a corrupt store silently disabled every suppression while reporting success), `aibom` published a symbol count of zero for an index it could not open, and `article-12-check` reported its high-risk scan complete over a truncated file set. Each is the same substitution the other fixes in this release share — an absent measurement resolved to a benign value instead of to *unknown* — and each now fails closed with the shortfall named in its own envelope. **How they were found is the more interesting part, and it produced a result worth recording even though nothing shipped from it.** Twenty-four defects of this class fixed in this release were mined out of git into a labelled corpus, each with its pre-fix code recovered by `git show <commit>~1:<path>`, so candidate detection rules could be scored on whether they actually catch defects known to have existed rather than on whether they sound plausible. The measurement produced a hard ceiling, stated before any rule ran: **8 of the 24 are closed to static analysis by construction**, because the compare-side family is a MISSING TERM in a key or a comparison and absence has no syntax — `sha256(label + "" + canon)` is a well-formed expression, and what makes it a defect is a term that is not there, which cannot be pattern-matched without already knowing which terms belong, that being exactly the knowledge the defective code failed to write down. One instance is unreachable by any code analysis whatsoever, since the defect lives in a docstring's shell command interacting with redirect semantics and no line of Python is wrong. The realistic ceiling is 10-12 of 24 at usable precision, and any headline recall above ~67% on this corpus would mean the detector was matching something other than syntax. A cardinality trap was flagged in the same pass: one corpus row anchors on a single file but its commit fixed 59 modules of identical shape, so counting sites rather than commits would let that one row dominate every recall number.
- **`roam budget` would have exited 5 for every user on their first run after upgrading, on a tree where nothing had changed.** Three unreleased fixes move numbers already written into `.roam/index.db`, and `roam budget` compares live metrics against the newest stored `snapshots` row. With shipped defaults its `No new cycles: max_increase 0` rule sees the one-time step and fails the build. **It is `cycles`, not `health_score`** — cycles go UP while `tangle_ratio` goes DOWN, because the fabricated edges had fused unrelated code into a single ~1,500-symbol strongly-connected component; breaking it yields many small genuine cycles with far fewer symbols trapped in each. The gate is a `metrics_version` stamp on the snapshot row, and the granularity was chosen by measurement rather than by argument: an earlier proposal versioned the health SCORE, but `collect_metrics` produces a **byte-identical** dict under the pre- and post-unification code on the same database, so the score formula moved nothing while the graph moved everything. The movement is entirely index-time, zero read-time — a score-version stamp would have caught none of it. Rows written before the stamp read as version 1 and every consumer now refuses to compare across a mismatch rather than comparing anyway: `budget` skips every rule and exits 0 with the reason disclosed; `health --baseline` suppresses the WHOLE delta through the same branch as a missing baseline (partial suppression would leave `cycles` reading as a real regression alongside phantom improvements on the LIMIT-capped columns); `fitness`, `alerts`, `forecast` and `bisect` truncate at the boundary and say so, which matters most for `bisect`, where a drop in edge count is classified as degradation and the fix would otherwise rank as the top culprit commit in every user's history; and the workspace aggregator stops silently averaging two definitions of health across a partly-reindexed workspace. Two consequences were found only by building it. The snapshot writer's `DELETE` is now **version-aware** — a same-definition row is a genuine duplicate, but a different-definition row is the only surviving evidence of what that commit scored under the old measurement, and deleting it would destroy the very data point a user needs. That change is only safe alongside a second one: `find_before_snapshot` selected `WHERE git_commit = ? LIMIT 1` with **no `ORDER BY`**, so once a commit can hold two rows it returned whichever SQLite reached first — rowid order, i.e. the oldest and stalest. And `open_db(readonly=True)` does **not** run `ensure_schema`, so the first post-upgrade run of every read-only consumer still sees the old table: naming the new column in SQL crashed `fitness` and `forecast` with `no such column` and silently dropped health scores in the aggregator, until a capability probe was added at the three sites that name it explicitly. The falsification pass caught its own headline test being **vacuous** — the seeded baseline passed against pre-fix code too — so both budget tests now share one regressing baseline and differ only in the version stamp; a temporary probe written without the new symbols then failed 10 of 10 against pre-fix code, reproducing the incident verbatim.
- **Six silent-degradation defects closed, and together they name a sharper mechanism than the one they were collected under.** They were gathered as instances of "a signal that could not be computed, rendered as one that came back clean" — which turns out to describe only three of them. Nothing failed to compute in the envelope cache, the snapshot comparison, or the attestation verifier; all three computed correctly and **compared the wrong things**. The mechanism that covers all six is narrower and greppable: *a value crosses a boundary without the identity of the producer that made it, and the receiver's default for "producer identity absent" is "producer identity MATCHES" rather than "unknown"*. Absent resolved to **equal** instead of to **unknown**, every time. The compile-envelope cache key omitted the tool version, so two versions hashed equal and a post-upgrade run could be served a pre-upgrade contract. A `snapshots` row with no metrics version compared as the same definition. The CGA predicate carried no graph-builder identity, so a verifier assumed the same builder — and "same builder" is precisely what licenses the wording *"edges changed since signing"*, which reads to a user as tampering rather than as the tool upgrade it was; that one looks inverse (it over-claims rather than under-claims) but is the identical substitution. `decode_text(errors="replace")` had no channel to say the decoded text was not the bytes on disk. And the disclosure ratchet's high-water mark had exactly one carrier — the file the documented regeneration command truncates before reading, so `--baseline > baseline.json` made the shell destroy the mark, `json.loads("")` raise, and the ceiling **silently reset** with its rationale replaced by a placeholder: the documented remedy for silent degradation, silently degrading. `--baseline` now writes the file itself and the redirect is gone from the contract. The fixes share a shape too, which is the real confirmation: each converts an implicit equality into an explicit UNKNOWN that fails closed. Two of the six were held back from an earlier commit for being **wrong rather than incomplete**, and both are fixed here: `cmd_migration_plan` hoisted the declaration of its warnings list out of the JSON branch while leaving the sole producer inside it, so the text branch echoed a guaranteed-empty list — silencing the scanner while disclosing nothing, a false clean of the false-clean detector; and `cmd_supply_chain` passed disclosures into the SARIF projector *and* wrapped the result, landing every marker in `toolExecutionNotifications` **twice**. Which half to keep was settled by execution rather than taste: the kwarg-only form fails when the projector itself raises, because only the wrap can salvage a floored `{}` into a valid zero-result run. An adversarial pass then found a hole in the new guard — a degenerate fix that discards **every finding** the moment any boundary degrades passed all five assertions, because they counted notifications and never pinned that the results survived. One added assertion closes it, and the lesson generalises: disclosing a degradation is not a licence to empty the document. Guards in seven new test files, each confirmed failing against its pre-fix code and each carrying a negative control; the CGA guard's control is load-bearing in the security sense — same builder version with changed edges must still fail as tampering, so the new version field cannot become an escape hatch for a real mismatch.
- **The disclosure scanner still truncated its own analysis silently, in three places, after being rebuilt to fail closed.** W1455 gave it three-valued per-file status, a published denominator, a positive control and a non-zero exit on unanalyzable — and left three UNCOUNTED caps inside the walk. A file whose analysis hit one was not examined; it was partially examined, and still resolved to `CLEAN`. The denominator counted FILES, not completeness. The fix came from enumerating every early exit and splitting them into two classes, which is the part worth keeping: **truncation** abandons an enumeration already in progress and must taint the verdict, while **resolution imprecision** models Python approximately and uniformly for every file — tainting on that would mark all 275 modules partial and destroy the signal. So the closure cap now records `budget.hit(...)` and yields `PARTIAL`; the `_MAX_DEPTH = 12` recursion cap was **deleted** rather than raised, because `visited` already guaranteed both termination and completeness and the number was only guarding the Python stack; and a third cap nobody had noticed — `for _ in range(3)` in `_carried_tokens` — now runs to convergence under `len(funcs) + 1`, which is a proof (one chain-edge per round) rather than a larger guess. Measured at HEAD, none of the three was load-bearing: each fired **zero** times, peak closure was 293 names against a 4,000 limit, peak depth was **10 against 12**, and the full scan ran 15.07s capped versus 14.73s fully uncapped — noise. Two levels of headroom is one helper-chain refactor away from silently truncating a shipped module, which is why the cap was removed rather than increased. Two planted corpora prove the caps were hiding real findings: a 20-helper chain and an 8-return chain through a shared bucket both came back `CLEAN` before and `VIOLATION` after. A fourth instance of the same shape was found in the same file: `_carried_mark` swallowed `OSError`/`ValueError` and returned the CURRENT count as the ratchet mark — an unreadable ratchet silently RAISED the ratchet, so a gate that lost its own history quietly got easier. Only `FileNotFoundError` is now a first generation; anything else refuses to emit. A coverage hole surfaced too: the one-level `glob("*.py")` meant `commands/pr_analyze/*.py` was in no bucket at all — not clean, not violating, not skipped, simply absent — now four `SKIPPED` records. In `cmd_secrets.py` the same audit moved `files_scanned += 1` below the read it was counting (it had been incremented before a `scan_file` that can swallow an `OSError` and return `[]`, inflating the denominator with files never opened), added a `read_errors` channel so `[]` stops being two-valued, and closed the partition with `files_filtered` / `files_undiscoverable`. Shadowed definitions — 28 across 11 of 275 modules — are published as `imprecision.shadowed_definition` rather than gated, and the one-module-deep closure limit is deliberately given NO key, because publishing an unmeasured limit as `0` is the same fabricated denominator in a different costume. Guards in `tests/test_w1459_uncounted_analysis_caps.py`: 14 of 15 fail against the pre-fix code, and the 15th exists to PASS — it runs the pre-fix scanner over both planted corpora and asserts they come back clean, which is what makes the other two behavioural rather than structural.
- **`roam impact` returned a byte-identical answer for unrelated symbols, because an unguarded case-insensitive fallback fabricated a quarter of the call graph.** `relations.py` resolved a reference in three steps, and step 3 was a case-insensitive lookup whose own comment reads *"VFP and other case-insensitive langs"* — with nothing checking the source file's language. It ran for every reference in every file, and this repository contains no FoxPro at all. The failure is worse than mis-resolution: steps 1-2 fail for any name not defined in the repository — stdlib, third-party, builtins — and step 3 then bound it to whatever indexed symbol happened to match case-insensitively. `Path` from `pathlib` alone manufactured ~3,435 edges onto an unrelated `PATH` constant in a throwaway dev script; across all case-fold edges, 85% had a source identifier with no exact-case definition anywhere in the index. Ground truth by re-parsing every Python source and checking real imports: **25.1% of edges were provably fake**, concentrated by kind at 86% of `reference` and 88% of `type_ref`. The user-visible consequence was measured before and after on freshly built indexes. Before: `roam impact PATH`, `roam impact open_db` and `roam impact json_envelope` all reported **18,331 symbols, 40.7%, risk high** — identical to the digit for a dev-script constant and the database-connection factory. After: 3 symbols / low, 14,444 / 32.0%, and 14,245 / 31.6% respectively. `roam deps` on that dev script reported **972 importers** where `grep` finds the filename nowhere; it now reports 11, the residual belonging to a separate simple-name matching path. Graph-wide, edges fell 6.45%, the largest strongly-connected component collapsed **1,484 → 123**, and the share of symbols claiming more than 30% of the repository as their blast radius went **53.5% → 7.0%**. That last number is why this mattered beyond a bad metric: `roam pr-risk` emits a risk factor at `blast_pct >= 20`, which essentially every symbol satisfied at a fabricated 40.7%, so a manufactured number was the named driver of the "critical" verdict a human reads before deciding whether a change is safe. The fix is one guard, chosen by ablation over the real graph rather than by argument: a competing candidate that dropped cross-file `reference`/`type_ref` edges removed 2.7x more edges and moved the pathology only 48% → 47%, while this one captures ~92% of it for 6.6% of the edges. It is expressed as a registry property — `LanguageExtractor.case_insensitive_identifiers`, declared `True` only by FoxPro — rather than a hardcoded extension check, so the next case-insensitive language never touches the resolver. One trap worth recording: the registry import must be **function-local**, because `roam.languages.registry` imports `roam.index.parser`, and importing anything under `roam.index` executes `relations` — a module-level import passed a smoke test and two full index builds before breaking collection for every test module that reaches `roam.languages` first; both import orders are now asserted. Guards in `tests/test_w1456_casefold_fallback_language_guard.py` (8 tests), and the negative control there is the interesting one: it **cannot fail against the pre-fix code by construction**, since the fallback existed then too, so its falsifier is deletion — stripping step 3 from the fixed code fails both FoxPro tests, which is the proof it is load-bearing. There was **zero local FoxPro resolver coverage** before this. Two findings beyond the fix: `type_ref` is the largest single loser at −67% of that edge kind, not `reference` as first assumed; and Apex and SQL are genuinely case-insensitive languages registered here that do **not** declare the flag, deliberately left unchanged so the behaviour stays inside the measured ablation, with a drift test that now fails the moment anyone adds one — making that a conscious decision rather than a side effect. **Migration:** on the next index, 5,557 of 44,948 `graph_metrics` rows (12.4%) change degree and **every** row changes `pagerank`, since it is normalised across the whole graph. Fitness baselines, health-score history and stored `pr-risk`/`attest` scores all shift. This is the third change today that moves stored metrics, after the health-score unification and betweenness seeding; they should be released together behind one baseline refresh.
- **The symbol-graph cache was keyed on `id(conn)` and had no invalidation at all, so a long-lived process served a stale graph after every reindex — measured at 100 out of 100 trials.** CPython reuses object ids once an object is freed, so a closed connection's cached graph could be handed to a different connection allocated into the same address. Reproduced in a fresh process, without pytest or its conftest cache-clearing: a strict `ABABAB` alternation collapsed 40 requests onto 3 connection ids and still produced **zero** wrong answers, because regular alternation pins each project to its own stable address — which is precisely why this stayed invisible — while the realistic **irregular** shape (`AAABBBAABBBBAAAB`, a session that works in one repo then switches) produced **8 wrong answers in 16 requests**, with two ids each shared by both projects and one project served the other's symbols. **The severity as originally framed is REFUTED, and that matters more than the finding.** Cross-project aliasing is not reachable through MCP as currently wired: `_run_roam` routes any `root` other than `"."` to a subprocess with a fresh empty cache, covering the 262 root-parameterised tools; the one in-process root-parameterised DB path (`roam_oracle_batch`) uses raw SQL and never builds a graph; and `batch_search`/`batch_get` are pinned to the server's cwd. An MCP server process is effectively pinned to one project, so "one project's structure answering another project's queries" does not happen there today. What IS live is broader in reach: **there was no invalidation of any kind**, so a server that caches a graph for its process lifetime keeps answering from it after an external `roam reindex` rewrites the database — 100/100 stale pre-fix, 0/100 after. `indexer.py` does call `clear_graph_cache()`, but only inside the indexing process, which is never the one serving. The key is now the resolved main-database path (via `PRAGMA database_list`, normcased) plus a change stamp, and the choice of stamp was itself corrected by measurement: an initial attempt used mtime and size, and **its own invalidation test caught it** — three successive commits left `st_mtime_ns` *and* `st_size` byte-identical while SQLite's header change counter moved 2 → 3 → 4, because Windows defers file-time updates. The stamp is now the header change counter plus size plus the `-wal` stat; `PRAGMA data_version` was rejected because it is explicitly not comparable across connections, and cross-connection lookup is the entire point. In-memory databases are never cached, since an empty path would make them all collide on one key. Cost is 89.6µs against a 460ms cold build over 44,688 nodes — 0.0195% — and the hit rate *improves*, because path-keying reuses the graph deterministically where id-keying only hit by address luck: on a repeated single-project sequence, builds drop from 4 to 1 and hit rate rises 60% → 90%. Guards in `tests/test_graph_cache_project_identity.py`, 2 of 3 failing pre-fix; the aliasing test asserts the cache is non-empty at the moment of aliasing (so conftest's clearing cannot mask it) and that at least one genuine id recycle occurred (so it cannot pass vacuously), and the third is a negative control asserting the same project twice returns the identical object, which blocks "disable the cache" as a fix. Three other `id(...)`-keyed caches were audited and are safe, one of them — `_FILE_TEXT_CACHE`, already path-keyed after a 2026-07 CI flake — carrying the "KEY DISCIPLINE" comment that is the direct precedent for this fix. This defect class has now bitten this repository three times: `tests/test_w1451_health_score_single_source.py` carries a hand-rolled `_reset_graph_cache()` whose docstring describes it exactly, and `workspace/aggregator.py` already loops raw connect/close over multiple repositories in one process, one import away from the same bug.
- **The scanner that enforces "a degraded signal must be visible" could not tell a clean file from one it failed to parse — and neither could the function whose job is proving coverage.** `scripts/scan_disclosure_asymmetry.py` backs the W1331 ratchet; `scan_file` did `except (OSError, SyntaxError): return []` and `enumerate_command_sites` did `except (OSError, SyntaxError): continue`. Reproduced by copying a command module, appending `def broken(:`, and scanning: `violations: []` and `enumerated sites: []` — **byte-identical to a clean repository**, with the zero-baseline test asserting `violations == []` and passing. A syntax error anywhere under `src/roam/commands/` silently removed that file from the gate AND from the coverage proof. `scripts/scan_snake_case_violations.py` carried the same `except SyntaxError: return []`, and its consuming test held a **private second copy of the scanner** with the identical handler — two independent places for one detector to stop detecting; the test now imports the script. Both are rebuilt to a three-valued contract: every file resolves to `CLEAN | VIOLATION | UNANALYZABLE | SKIPPED` with a reason, and no path returns an empty result for a file it could not read. The scan publishes its own denominator (`files_parsed` / `files_unanalyzable` / `files_skipped`), which the consuming test asserts **separately** from `violations == 0`, so a parse regression fails loudly rather than quietly. Exit codes distinguish the cases the old scanner collapsed: `0` clean, `1` violations, `2` unanalyzable, `3` scanner untrustworthy — the last covering both a broken self-check and zero files parsed, because zero coverage is not zero violations. Every exception in the walk, including a crash inside the analyzer, is caught per file and recorded rather than aborting or silently continuing. The piece that makes this durable is a **positive control run on every invocation**: a planted sentinel command whose disclosure asymmetry must be found, paired with a symmetric twin that must stay silent — so "always fire" fails the control exactly as "never fire" does. If the sentinel is not detected the run reports BROKEN and exits 3 even with zero violations. The reasoning behind it is the general lesson: **without a positive control, "0 findings" and "the detector stopped working" are the same output**, which is precisely the defect class the scanner exists to prevent. Zero matches is now reported as `checked 275 files, 0 violations, 0 unanalyzable, positive control OK` — the disclosure is the count, not the silence. Also fixed: a dead skip-list in the snake-case scanner that compared `rel_parts[0]` against directory names while always scanning under `src/`, so it had never skipped anything; the baseline regenerates byte-identical, confirming the rewrite is finding-neutral, and an old-loop-versus-new-report comparison over the real tree returned an identical violation set. **Measured exposure: zero.** 275 command modules parsed, 0 unanalyzable; 625 files for the snake scanner, 0 unanalyzable. The gate was not lying — it was incapable of telling you if it were. Guards in `tests/test_w1455_scanner_fails_closed.py`, 24 of 24 confirmed failing pre-fix, plus 14 pre-fix failures across the consuming tests. Finally, the repaired scanner immediately caught a real asymmetry in a command shipped earlier the same day: `roam review-verify` emitted `partial_success` to its JSON envelope and nothing to text, so a human or a log-scraping pipeline read a NON-passing verification exactly as it read a clean one. The first fix disclosed to text alone — and the scanner caught **that** inversion too, which is the point of checking both directions; the marker is now built once, above the branch, and carried by both.
- **`roam diff --fitness` reported `[PASS]` for rules it had never checked — including `trend`, a rule type the system fully supports — and `roam attest` baked those false passes into SIGNED evidence.** `cmd_diff._collect_fitness_violations` dispatched with an `if/elif` chain over `dependency`/`metric`/`naming` only. Anything else fell through to `violations = []` and was then recorded `status: "PASS"`. Unlike the sibling defects fixed alongside it, this was not a silent drop: the envelope positively asserted the rule had been checked and had passed. Measured against one rule file where `roam fitness` reports dependency FAIL, naming PASS, **trend FAIL** and a typo'd type ERROR, `roam diff --fitness` printed `4 rules, 3 passed, 1 failed` with `health-must-not-regress [PASS]` — a `trend` rule that had never once been enforced on this path — alongside `typoed-rule [PASS]`, a type nothing can enforce. `cmd_preflight` held its own copy of the same hole (its exception path was closed separately; this is the unknown-type path), so a typo'd rule vanished from `rules_checked` entirely: the user wrote 2 rules, the envelope said `rules_checked: 1`, `severity: OK`, exit 0. **The amplifier is the part that matters**: `cmd_attest._collect_fitness_evidence` consumes `_collect_fitness_violations`, so every false PASS was being written into signed attestation evidence — a cryptographically vouched claim that a rule passed when it was never run. Fixed by the same change, and verified emitting honest rows. Both sites now disclose rather than fail closed, and that was decided rather than copied: `roam diff --fitness` **already exits 0 on a genuinely FAILING rule** (measured), because `diff` is an informational blast-radius report while `fitness` is the gate — so borrowing the gate's exit-1 would make a config typo exit non-zero while the architectural violation it was written to catch exits zero, which is strictly stranger than the bug. The decisive second reason is mechanical: `_collect_fitness_violations` is a library function called behind `except Exception`, and `SystemExit` derives from `BaseException`, so it would escape that guard and take `roam attest` down on a typo — pinned by a test asserting the collector never raises `SystemExit`, with the exit-0-on-real-FAIL premise pinned in both files so the decision is revisited if that ever changes. The `if/elif` is gone: `_SCOPED_CHECKERS` is now a narrowing table over `_CHECKERS`, membership goes through the shared `_resolve_checker`, and the fall-through is inverted — a registered type with no diff-scoped variant runs the global checker and carries `scope: "repository"` (a `trend` rule is inherently unnarrowable, since snapshot history has no per-file projection, and disclosing the scope stops a repo-wide FAIL reading as "this diff caused it"), while an unknown type produces the ERROR row with `scope: "not_evaluated"`. `cmd_preflight` routes unknown types into the `errored_rules` bucket that already existed rather than opening a parallel channel. Both files carry AST shape guards forbidding literal type comparisons and local `_CHECKERS.get` lookups, plus add/delete drift tests proving that registering a checker teaches each consumer and deregistering one turns it ERROR — so a fourth `elif` cannot pass. Suites in `tests/test_w1453_diff_fitness_dispatch.py` (22) and `tests/test_w1453_preflight_unknown_rule_type.py` (18): **29 of 40 confirmed failing pre-fix**; the 11 green in both states are exactly the negative controls, the exit-0 premises, the shared-registry invariant, and the vacuously-true half of a drift pair whose sister fails. One related drift corrected while looking: `output/sarif.py`'s `fitness_to_sarif` docstring hand-listed three rule types and omitted `trend` — harmless at runtime, since `rule_id` derives from the value, but the same hand-maintained-list hazard one layer down.
- **`roam health --baseline` reported progress that never happened, because the score was implemented twice and the two copies disagreed.** `cmd_health` computed one number and `commands/metrics_history.collect_metrics` computed another; the second is what gets STORED in `snapshots`, and `--baseline` compares today's first against yesterday's second. Measured on the same database at the same instant: **score 71 vs 64, `tangle_ratio` 0.0 vs 3.6**, and on this repository's real stored snapshot the command reported verdict **OK, score +2, "38 cycles fixed"** — on a repo that had actually **regressed**. Post-fix the same comparison reads **BAD, −5, +1 new cycle**. Not a crash: a confident false report, in the one feature whose entire purpose is detecting real change. The divergence was isolated to a single numerator. `total_symbols` and the graph's node count are the same population, so `fingerprint`'s 0.0359 and `collect_metrics`' 3.6 were one measurement differing only by ×100 — but `cmd_health` filtered its cycle set to **actionable** SCCs, of which 0 of 39 qualify, yielding 0.0 while the same envelope simultaneously reported `cycles_total: 39`. Self-contradictory in one document. `collect_metrics` is the correct one, and not merely because it is the stored one: its numerator matches its own published definition ("symbols inside non-trivial SCCs", with no actionability qualifier) and matches `roam fingerprint`. Both units are retained deliberately, because each is load-bearing somewhere — percent for `.roam-gates.yml`'s `tangle_max`, the sigmoid scale and the stored column; fraction for `fingerprint --compare`'s `max_range: 1.0` — with the definition now stating which is which and a test asserting `health_pct == fingerprint_frac × 100`. The fix is a new `roam.quality.health_score` module owning tangle ratio, signal derivation, factor composition and the geometric mean, called by both sites, plus a guard that fails if the two ever diverge again — which is the part that makes it permanent, since two copies of a metric will always drift and nothing was comparing them. Four more divergences in the same family were found while unifying: `--baseline` compared `len(actionable_cycles)` against the stored all-SCC `cycles` column; its `dead_exports` delta used a Python-only `test_*` check against a stored column computed with the canonical multi-language predicate, a standing false delta on every Go, Java and JS repo; `collect_metrics` omitted the 10% imported-coverage factor that `cmd_health` applies; and it wrote `(avg or 10)/10.0`, silently rewriting a **measured file-health average of 0.0** into a perfect factor — the same absence-versus-zero conflation, one line further down. **User-visible consequence, stated plainly: this repository's health score moves 71 → 64**, and projects configuring `health_min` between 65 and 71, or any `tangle_max`, may newly fail their gate. The gate is now honest. Other `snapshots` readers are unaffected: `collect_metrics` output is byte-identical pre and post, so `trends`, `forecast`, `alerts`, `bisect` and the workspace aggregator keep reading a column whose value and meaning are unchanged, with no historical discontinuity — the sole exception being projects with imported coverage data, which see a one-time step. `roam understand` and `roam dashboard` read `collect_metrics` and therefore disagreed with `roam health` by 7 points; they now agree exactly, and `test_understand_health_band_parity.py` was strengthened from band-agreement to score-equality, closing a duplication its own comment had been tracking as an open follow-up. Regression suite in `tests/test_w1451_health_score_single_source.py`: **8 of 12 failing pre-fix**, verified in a detached worktree rather than by stashing, since the tree is shared. The negative control requires a clean fixture to score 60-100 and requires that adding a cycle LOWERS the score by the SAME amount at both call sites — so "make both return 0" cannot pass.
- **The `pre-commit` hook was written in bash but declared `#!/bin/sh`, so on Linux it aborted before running a single check.** Line 143 tested `[ -n "${STAGED_PY// /}" ]` — a bash pattern substitution. Under dash, which is `/bin/sh` on Debian, Ubuntu and every Linux CI runner and build lane this project uses, that is not a subtle difference in behaviour but a parse error: `dash: 1: Bad substitution`. Verified directly on a Debian host where `/bin/sh -> dash`. Because it aborts at parse time it took the WHOLE hook down, so the ruff gate, the surface-count check, the changelog-drift check and the internal-index check never ran on any Linux commit — and the failure was total rather than partial, which is why nobody saw a partial-gate symptom to investigate. It survived because of where it was written: on Windows, git-bash makes `/bin/sh` bash, so the construct works perfectly on the author's machine and nowhere else. Replaced with `[ -n "$(echo $STAGED_PY)" ]`, which relies only on POSIX word-splitting and gives identical results on all four inputs that matter (empty, whitespace-only, one file, many) — confirmed under real dash, not by reasoning about it. The guard is a STATIC scan rather than a runtime one, in `tests/test_w1454_githooks_posix.py`, and that choice is the point: running `sh -n` on the Windows host where this was authored invokes bash, which parses the broken line happily. **A portability bug cannot be caught by executing on the platform that lacks the problem**, so the test scans every `#!/bin/sh` hook for thirteen constructs POSIX sh does not have (`${var//}`, `${var^^}`, `[[ ]]`, `=~`, `<<<`, `mapfile`, `local -a`, and the rest), skipping any hook that declares bash honestly. It ignores comments and single-quoted spans so a comment explaining a bash-ism cannot trip it. Two negative controls make the scanner falsifiable in both directions: one feeds it the exact line that shipped and requires a hit — without it, a scanner whose regexes matched nothing would report the codebase clean — and one feeds it the POSIX replacement and requires silence. Confirmed failing against the pre-fix hook. Found while building a lander that runs each repo's own pre-push gate on a Linux box, i.e. by finally executing our own tooling somewhere other than the machine that wrote it.
- **Betweenness centrality was sampled without a seed, so every `roam index` produced different numbers for unchanged code — and those numbers feed the feature whose whole job is detecting change.** `nx.betweenness_centrality(G, k=k, normalized=False)` approximates over `k` randomly-chosen pivots; with no `seed=`, each run drew a fresh sample. Measured on a copy of this repository's index (44,688 nodes, k=1,056 pivots = 2.36% sampling), across three back-to-back runs on identical input: **3,904 of 44,688 nodes changed value**, median relative change 7.5% on those that moved, the positive-betweenness population swung 3,446 → 3,566 (Jaccard 0.759, ~25% churn), and the p70/p90 severity bands moved +11.5% and +9.4%. Every one of those differences is a `roam health --baseline` delta that no code change caused. The honest nuance, because it makes this smaller than it first looks: **the head is stable and the tail is not.** Top-10 and top-15 were identical in membership and order across all three runs, and since `_BOTTLENECK_LIST_LIMIT` is 15 and all fifteen sit ~70x above p90, this repository's displayed bottleneck table was already effectively deterministic. What genuinely moved with no code change was the betweenness value printed on each row (−6% to **+54%** on individual top-20 symbols), the disclosed population in `showing_top_15_of_N`, the p70/p90 figures in the JSON report, and all 44,688 stored rows. On a flatter repository whose top-15 are not saturated above p90, severity classifications flip outright. Fixed with a module constant `BETWEENNESS_SEED = 1` at both call sites — a fixed seed rather than a repo-derived one on purpose: a content-derived seed would redraw the pivot set on every code change and fold fresh sampling noise straight back into the baseline diff this is meant to protect, while also breaking comparability across clones. End-to-end on the real 44.7k graph, two seeded runs now differ on **0 of 44,688** nodes. A sibling was found and fixed in `graph/simulate.py`, and its shape is the more instructive one: its `bottlenecks` COUNT was perfectly invariant across eight unseeded runs (104 every time) while the MEMBERSHIP churned to a Jaccard of 0.29 — roughly 71% different symbols behind an unchanging number. A stable aggregate concealing near-total content churn is its own species of false clean, and a count alone would never have revealed it. Audited the rest: `partition.py`, `cmd_cut.py`, `clusters.py`, `spectral.py`, `cmd_split.py` and `cmd_why.py` already seed; `cycles.py` and the exact no-`k` paths are deterministic by construction; and the `n <= 1000` branch passes `k=n`. An earlier draft of this entry claimed that made small graphs safe, on the strength of six unseeded runs coming back bit-identical locally. **CI refuted it on the first run — 4 distinct results in 4 attempts.** `k == n` means every node is *sampled*, not that sampling is skipped: `random.sample` still returns a different PERMUTATION each call, and floating-point addition is not associative, so the same contributions accumulated in a different order differ in the low bits. There is therefore no graph size at which an unseeded call is safe, which strengthens the fix rather than narrowing it — and the local six-run agreement was luck being mistaken for evidence. Guards in `tests/test_w1452_betweenness_determinism.py` (11 tests): 5 fail against a behaviour-only revert that keeps the constant but strips `seed=`, the negative control is measured rather than hoped (unseeded sampling produced **12 distinct results in 12 runs**, so the falsifier is not luck-dependent), a different-seed-gives-a-different-answer check rules out a draw-insensitive test graph, a boundary guard stops the fixture silently dropping below the sampling threshold, and an AST drift guard scans all of `src/roam` for any future `betweenness_centrality` call that passes `k` without `seed`. **Migration consequence, stated rather than discovered later:** the next `roam index` changes every stored betweenness value on any repository above 1000 symbols, so the first post-fix `--baseline` run shows a one-time step change in bottleneck values, band edges and population. That step is the fix landing, not architectural drift.
- **A typo in a fitness rule's `type` silently disabled that rule and left CI green.** The dispatcher did `checker = _CHECKERS.get(rule.get("type", "")); if checker is None: continue`, and since `rules_checked = len(rule_results)`, a rule with an unrecognised type vanished from BOTH the numerator and the denominator. The loader validated only that a rule was a dict, so nothing anywhere checked `type` against the four registered checkers. Measured with two eligible rules — one `type: layer` (the term the module docstring itself ADVERTISED) and one `type: dependancy` (a typo) — the command emitted `rules_checked: 0`, verdict `all 0 fitness rule(s) pass`, and **exit 0**. The user wrote two architectural rules, the tool enforced neither, and reported success. The W1051 comment three hundred lines below already stated the intent — warnings exist "so the agent doesn't see a green verdict that silently dropped half its rules" — but covered loader drops only, never dispatch drops. Fixed by failing closed AND disclosing, not one or the other: the load-bearing signal here is the exit code (`roam fitness && echo "Architecture OK"` ships in the `--init` template), so disclosure alone would leave the gate off while CI stayed green — the exact shape W1332 was written against. An unrecognised type now produces a rule-result row with `status: "ERROR"` and a `difflib` did-you-mean (`'dependancy'` → "Did you mean 'dependency'?"), `rules_checked` counts every rule the user wrote, `_rule_counts` returns `(passed, failed, errored)` under the invariant `passed + failed + errored == len(rule_results)`, and `_finish_fitness` exits 1 on any errored rule **ahead of every baseline branch**. Two deliberate sub-calls, both pinned by tests: the ERROR row is kept OUT of `all_violations`, because a config typo is not a per-location finding and routing it there would let `--write-baseline` bless the typo into permanent silence; and the errored check precedes the baseline branches for the same reason. The structural half matters more than the fix: `_resolve_checker()` is now the single membership test that BOTH loader and dispatcher call, with `_known_rule_types()` reading `tuple(sorted(_CHECKERS))`, so registering a checker teaches both halves and no second hardcoded list can drift — proven behaviourally by tests that add AND delete a `_CHECKERS` key and assert the loader follows in both directions. The verdict string was also being re-derived from two copies of the same three-branch conditional in the text and JSON paths; both now call one `_fitness_verdict()`. The docstring's false "layer enforcement" claim — the trap that produces `type: layer` in the first place — was removed, along with the same false advertisement in the `roam_fitness` MCP tool description and `docs/mcp-tools.md`. Regression suite in `tests/test_w1450_fitness_unknown_rule_type.py`: **15 of 18 assertions confirmed failing against the pre-fix code**. The 3 green in both states are the two negative controls (valid rules still checked, still PASS, still exit 0, no new envelope fields; and a mixed config still evaluating its valid rule) plus one direction of the drift test that is vacuously true pre-fix — its sister case fails there, which is the point.
- **`roam preflight` turned a fitness checker that CRASHED into `Fitness: all 1 rules pass [OK]`.** The rule checker was wrapped in `except (ImportError, re.error, sqlite3.DatabaseError): violations = []`, and status was then derived as `PASS if not violations else FAIL` — so a rule whose checker raised produced an empty violation list, which is also exactly what a genuinely clean rule produces. The rule stayed counted in `rules_checked`, so the text renderer reported a full pass and `_fitness_severity(0)` returned `OK`. The zero-rule case *was* disclosed ("no rules configured"); the exception case was not, making it a half-fix. Proven by divergence rather than by inspection: given a rule with the malformed pattern `handle_([`, `roam fitness` exits 1 with `unterminated character set at position 8` — loud and fail-closed — while `roam preflight` exited 0 with `status: PASS, violations: 0, severity: OK`, byte-identical to a healthy-rule control run. Same rule file, same checker, opposite verdicts; that disagreement is what proves it was fail-open rather than a deliberate choice. The `except` is retained — one broken rule must not take the other N-1 down — but a caught exception now yields `status: "ERROR"` with `violations: None` rather than `0`, mirroring `db-check`'s `count: None`, because a floored zero reads as "ran, measured nothing" while `None` says no measurement happened. New `rules_evaluated` / `rules_errored` / `errored_rules` fields report the shortfall, `rules_checked` deliberately keeps meaning *rules attempted* so it still agrees with `len(rule_details)`, and severity is floored off `OK` to `WARNING` when any rule errors — otherwise an all-ERROR section rolled into `_overall_risk` as clean. Markers reach `summary.warnings_out` and flip `partial_success` through the existing plumbing, and preflight's text mode gained `echo_text_warnings` (stderr, so stdout stays byte-identical) since it previously disclosed nothing at all. One structural fix beyond the defect: four hand-rolled three-bucket warning concatenations were replaced by a single `_all_warnings_out()`, so a newly added bucket can no longer reach three call sites and silently miss the fourth — which is how a disclosure channel quietly loses a branch. Regression suite in `tests/test_w1449_preflight_checker_exception.py`: **13 of 15 assertions confirmed failing against the pre-fix code**, including an AST guard that names the flooring line. The 2 passing in both states are the divergence pin and a load-bearing negative control asserting **byte-equality** of the healthy-rule text `Fitness: all 1 rules pass [OK]` — without it, a "fix" that simply failed everything would have scored green.
- **`roam health` reported two SQL `LIMIT` constants as if they were populations, and `issue_count` was literally their sum.** The god-component and bottleneck sections render a top-N list, but the caps were inline literals passed straight to `LIMIT ?`, so once saturated the returned list length was indistinguishable from a measured count. Measured on this repository: `summary.god_components: 50` against a true population of **553**, `bottlenecks: 15` against **3019**, and `issue_count: 65` — exactly 50 + 15 — all published with `summary.partial_success: false` and an empty `preserved_list_truncations`, i.e. an explicit claim of completeness. The envelope contradicted itself inside one document, printing `bottleneck_thresholds.population: 3019` beside a bottleneck count of 15. Neither threshold is reachable in practice, so this is not a quirk of one repository: the 50th-ranked symbol here has degree 142 against a threshold of 20, and betweenness is unnormalised (raw shortest-path counts) so the 15th-ranked value is ~1.5e7 against a threshold of 0.5 — the cap, never the threshold, is what stops the cursor. Nothing upstream could have caught it, because `output.formatter` records only the truncations *it* performs and this one happens in SQL, beneath that layer; the disclosure was therefore made in good faith. The caps are now named constants, the god section publishes a measured `god_component_thresholds.population` counted against the same `> min_degree` predicate its items are filtered on (the bottleneck section already published its population but never said the list was capped, so both now carry `list_limit` too), and a saturated list raises `partial_success` with a marker naming the true population. Text mode says "top 50 of 553 god components" rather than "50 god components". Disclosure requires BOTH cursor saturation AND a population exceeding the cap — not saturation alone: `TOP_BY_DEGREE`/`TOP_BY_BETWEENNESS` carry no `WHERE` clause, so on any project with at least `limit` indexed symbols they return a full list even when nothing clears the qualifying threshold, and a full list is not a truncated one. That distinction was found by a negative control in the new suite firing against the first version of this fix, which would otherwise have shipped a false truncation claim on every small clean project. Guards in `tests/test_w1448_health_list_cap_disclosure.py` seed a population larger than the caps rather than relying on this repository staying large, and include a control asserting the lists are still capped (so "disclose" cannot degrade into "return everything") and one asserting silence on an unsaturated project. **Deliberately NOT changed, and recorded rather than quietly folded in:** `issue_count` and the health score still derive from the capped, classified sample. Those signals count *actionable* items — a per-item classification that only ever ran on the fetched rows — so substituting a raw population would change what the metric means rather than uncap it, and the score's scales were themselves tuned on the now-falsified premise (a code comment reads "this repo had 50 god components total"; 50 was the LIMIT). Correcting those needs classification over the full population plus a stated re-tune, tracked separately.
- **`roam verdict` — the gate itself — returned exit 0 and `all_required_passed` on a bundle that recorded no passing check, because satisfaction was written as a denylist.** The previous fix in this area (W1441) closed the status-less-record fail-open inside `proof_bundle._extract_executed_checks` and expressed the rule downstream as `status != "unverified"`. A denylist only refuses the one value it was taught to distrust, so every other value satisfied a required check. Measured against the shipped 13.10.0 binary with a contract requiring `pytest -q`, all four of these returned exit 0 / `pass` / `all_required_passed`: a record with status `"skipped"`, a record with **no status field at all** — the exact shape W1441's own commit message names — a record with status `"Unverified"`, where capitalisation alone defeated the comparison, and a record with status `null`. Two things made it invisible. First, `proof_bundle.validate_v1` *does* reject those shapes against the closed `CHECK_STATUSES` enum, but `cmd_verdict` never calls it, so two validators in the same codebase disagreed about the same bundle and nothing compared them. Second, the CLI read `body["tests_run"]` raw rather than through the normaliser `build_proof_bundle` uses, so W1441's hardening sat one layer below the entry point a caller actually invokes. The fix inverts the direction rather than extending the list of distrusted values: satisfaction is now an **allowlist on `"pass"`**, which fails closed on statuses nobody has invented yet — something a denylist cannot do by construction. A record carrying a status outside the enum no longer reports as "not run" but gets its own blocker code, `required_check_status_invalid`, carrying the offending value, since "you never ran this" and "you ran it and recorded no outcome" call for different actions from the agent reading the reason. `fail`/`error` are deliberately excluded from the new blocker so the failure loop keeps reporting them precisely instead of one defect surfacing as two unrelated reasons, and the legacy `tests_run` path now shares the one normaliser so the two halves cannot drift apart again. Regression suite in `tests/test_w1447_verdict_status_allowlist.py`: 13 of its 18 assertions were confirmed failing against the pre-fix code, and the 5 that pass in both states are the negative controls — a genuinely passing check must still pass, a failing one must still be reported as failed rather than as missing, `unverified` must keep its more specific code, and every member of `CHECK_STATUSES` must agree with the allowlist so the schema enum and the gate cannot diverge again. Those controls are load-bearing: without them, a "fix" that blocked unconditionally would have made the suite green. **Known limit, unchanged and stated rather than quietly patched:** a bundle carrying no `verification_contract` at all still returns `pass`, because an empty required-set is vacuously satisfied. That is the already-disclosed "the gate reads a contract from the bundle the agent authors" limit, and closing it is not a matter of tightening this predicate — it requires an authority outside the agent's write boundary to supply the contract, which is a separate, deliberately open design question.
- **`roam taint` re-scanned the entire corpus once per rule, per file, per name — 292,221 whole-file regex passes of which 98.1% could not match, making it 94% of the `reachability-triage` service report.** `_scan_hits` compiled a boundary-anchored pattern for every candidate name and ran `finditer` over the whole masked file text, three times per (rule, file) — once each for sources, sinks and sanitizers. The cost was the corpus length multiplied by every name any Python rule declares, and almost all of it bought nothing: the overwhelming majority of those scans traversed a file that does not contain the name anywhere. The fix is a single substring pre-check, and it is sound for a reason worth stating precisely — `_dotted_name_pattern` compiles to `(?<!\w)` + `re.escape(name)` + `(?!\w)`, which only ADDS boundary lookarounds and never widens what can match, so a literal occurrence of the name is a strict *necessary* condition for a match and `name not in masked_text` cannot skip a hit the regex would have found. Measured on this repository (4,898 files / 44,412 symbols, warm index): `taint` **87.4s → 15.6s**, `service-report --type reachability-triage` **96.4s → 28.1s**, with 286,659 of the 292,221 scans eliminated and the emitted payload byte-identical outside volatile `_meta` timestamps. Because the argument, not the arithmetic, is what makes this safe, `tests/test_w1446_taint_scan_prefilter.py` proves equivalence by *differential execution* against an oracle that reimplements the pre-fix loop, and separately pins the two regex properties the pre-check depends on: that the escaped literal still appears in the compiled pattern, and that the pattern is not case-insensitive. If anyone later loosens `_dotted_name_pattern`, the necessary-condition argument breaks and the scanner starts silently *missing* taint findings — a failure with no symptom — so those guards fail loudly instead. Found by measuring a dogfood defect (M8) that had been carried as an unmeasured estimate since 2026-07-15; every specific in the original triage was wrong. `due-diligence` was recorded at "~130s" and is 87.5s, so it was never the headline offender; `reachability-triage` was recorded at "~41s" and had **regressed to 96.4s**, unnoticed because M8 was pinned only as `pytest.mark.slow` with a 300s ceiling and no perf assertion; and the guessed root cause (`dead`/`clones`/`complexity` fan-out) was wrong, `complexity` being 0.42s of it. The registry entry has been rewritten with measured numbers. One measured cost is deliberately left open and recorded rather than fixed: `due-diligence` extracts only 1.38x from its 3 workers (120.5s serial against 87.5s wall, ideal makespan ~47.5s) because three concurrent CLI children each re-open a 97MB `index.db` — raising the worker cap would add contention over the same file rather than remove it.
- **Every mined import law fired backwards on every `src/`-layout project: the conventional internal import was flagged, and the one spelling that cannot import at runtime was cleared.** The miner bucketed *file paths* read from the index (`src/roam/db/connection.py` → `src/roam`) while the checker bucketed *module paths* read from a diff (`from roam.db.connection import open_db` → `roam/db`). Under a src layout those two namespaces never intersect, so the polarity inverted: `from roam.db.connection import open_db` — what every test in this repo writes — was reported as a layering violation, while `from src.roam.db.connection import open_db`, naming a package that does not exist and fails at import, passed clean. Measured on this repository over the trailing 29 non-merge commits: **47 violations raised, 0 of them true** — conventional `from roam.<pkg> import <name>` lines, the repo's own mandated test-root helper import, one stdlib module absent from a hand-rolled allowlist, and one line of prose inside a docstring that the diff parser read as an import. After the fix, the same 29 commits raise **1**, and that one is factually correct: a test that really does import across the boundary its law describes. The existing coverage missed all of this because its fixture diff imports `from src.forbidden.module import bad_helper`, a spelling that includes the source root and that no src-layout project ever writes. Both halves now bucket through one new module, `roam.laws.namespace`, which re-expresses a file path in the namespace an `import` statement can actually name. Source roots are *detected from repo layout* rather than special-cased on the name `src`: a directory that holds packages, holds no loose modules of its own, and is not itself importable — so `lib/`, `python/` and a monorepo's `packages/app/src` fall out of the same rule, and a repo that declares no source root (any flat layout, any non-Python tree) is bucketed exactly as before and does not move. Three further false-positive sources in the same checker were closed while the measurement was open: the stdlib allowlist became `sys.stdlib_module_names`, so the missing `statistics` cannot recur with the next module anyone adopts; "is this import ours?" is now asked of the checkout — does a directory or module of that name exist at the repo root or under a source root — instead of of a hand-maintained list of third-party names; and relative imports are resolved against the importing file's own directory, since `from .helpers import x` names a sibling, not a top-level package `helpers`. A fourth, found while verifying the first: `lstrip("./")` strips *characters*, so `.github/scripts/gate.py` bucketed as `github/scripts` and the law mined from that directory named a directory that does not exist. The two `xfail(strict=True)` tests shipped in `tests/test_w1439_laws_import_law_src_layout.py` flipped to XPASS on the fix and are now ordinary assertions; the mechanism, the source-root rule, and each false-positive class are pinned in `tests/test_w1440_laws_namespace_contract.py`, including an end-to-end mine-then-check round trip over a synthetic src-layout project that is not this repository. Every new assertion was run against a deliberately broken version of the property it protects and confirmed to fail there.
- **Four of the eleven laws this repository mined could never have fired, and were mined anyway.** `added_symbols()` labels every declaration it recognises as `function` or `class`, so the mined `constant`, `method`, `property` and `variable` naming laws matched nothing on every diff — a diff flagrantly violating all four reported clean. Mining stopped for kinds the checker cannot observe, rather than teaching the checker to observe them: `constant` is the case that settles it, because the Python indexer classifies a symbol as a constant *iff its name is upper-case*, which makes "constants must be UPPER_SNAKE" true by construction and unfalsifiable no matter how good the checker gets; `method` is already covered by `snake_case_functions`, since the diff regex labels every `def` a function; and `property` and `variable` need decorator and scope tracking that a regex over added lines cannot honestly do. The skip is not a hardcoded list — the miner reads `roam.laws.checker.CHECKABLE_SYMBOL_KINDS` and declines to mine anything outside it, so widening the checker re-enables the corresponding laws automatically and the two halves cannot drift apart again. The dropped kinds are disclosed in the `laws mine` envelope as `skipped_naming_kinds` with a reason, so a shorter law list arrives with an explanation rather than as silent attrition. A guard test fails if `CHECKABLE_SYMBOL_KINDS` is ever widened past what `added_symbols()` actually emits, which is the exact shape of the defect being removed.
- **`roam cycle-break` printed nothing at all on a corpus with no symbols indexed, and `roam clusters` reported "no clusters detected" for it.** The empty-corpus branch in `cycle-break` had a `if json_mode:` with no `else` — text mode produced zero bytes of output and exit 0, which a caller reads as "there are no cycles to break". `clusters` printed the same verdict for an unbuilt index as for a genuinely well-modularised repository, while its JSON envelope carried `state: empty_corpus`. Both now disclose through the shared `formatter.echo_text_empty_corpus()` helper, and `cycle-break` gained the missing verdict line. Measured on a real 0-symbol index; the populated-fixture control stays silent. Reviewed the other three `empty_corpus_state` baseline entries in the same pass — `cycles`, `alerts` and `debt` already name the empty corpus in their text verdict, so they are instrument false positives and were left alone rather than double-disclosed. Ratchet baseline 54 → 52.
- **Thirteen more W1331 disclosure asymmetries closed, this time on the commands a human runs to answer "is this safe?".** `roam adversarial` is the sharpest: with its severity classifier raising, the command printed `VERDICT: No architectural challenges found -- changes look clean` while the JSON envelope named the failure — a review command certifying clean off a scorer that never ran. `roam doctor` skips any check helper that raises, so a raising check produced no PASS line, no FAIL line, and no `failed_checks` entry: the health report a user reads to answer "is my install broken?" simply omitted it and the verdict counted one fewer check. `roam vuln-reach` reported `0 reachable vulnerabilities` after the symbol graph failed to build. `roam health` rendered a failed graph build as `Tangle: 0.0% (0/44309 symbols in cycles)` — the best possible score — with no note. `roam cga verify` printed `see warnings_out` in its error list while never printing `warnings_out`. `roam sbom` on this repository already discloses a real, un-injected truncation (`reachability_scan_truncated: file cap reached; unimported/phantom verdicts may under-report`) that only the JSON envelope carried. Also fixed: `audit-trail-verify`, `evidence-doctor`, `flag-dead` (an unreadable `--config` silently disabled known-stale matching), and a scope bug found while proving the `cga` fix — the W489-A `qualified_only` lint bucket was *built* inside the `if json_mode:` branch, so the text tail could not have disclosed it even in principle. Same template as the previous entry: markers to STDERR via `formatter.echo_text_warnings()`, stdout byte-identical.
- **`failed_checks` demoted from the W1331 gated vocabulary to the measured-only set, and four buckets renamed to the codebase's own convention.** Both live `failed_checks` instances were run and found to disclose in every branch, just not under that name: `adversarial` appends the errored-check list to the VERDICT line verbatim, and `doctor` renders one `[WARN]`/`[FAIL]` row per entry plus a count. Gating an envelope KEY NAME there measured spelling, not disclosure — the same reasoning that already exempts `partial_success`. Separately, `rules`, `check-rules` and `flag-dead` were flagged while their text branches *already* printed the warnings; their buckets were named `_rules_warnings` / `deduped_warnings` / `_known_stale_warnings`, outside the `*_warnings_out` convention the instrument matches on, so the disclosure was invisible to the scan. Renamed rather than re-disclosed. The ratchet baseline drops 67 → 54.
- **The anti-leak gates reported clean over content they had never correctly read: an empty file inventory and any UTF-16 file.** Two defect shapes, both proven by end-to-end reproduction against the real scanners before anything was changed, both silent — neither produced a diagnostic distinguishing "found nothing" from "read nothing". First, the whole-tree scanners shape their verdict as "hits among the enumerated tracked files" and never checked that the enumeration produced anything. `git ls-files` honours `GIT_INDEX_FILE`/`GIT_DIR`/`GIT_WORK_TREE`; pointed at another index it prints nothing and exits 0, and both `scripts/scan_internal_language.py --all` and the CI gate `tests/test_no_internal_language.py` then passed a tree holding a plainly-visible leak. That matters precisely because these run from `.githooks/pre-commit` and `.githooks/pre-push`, exactly where Git exports repository-local `GIT_*` controls — `.githooks/pre-push` already unsets them, so a push through the hook was defended while the CI invocation and any direct run were not. Both enumerations now strip `GIT_*` and fail closed on an empty answer, guarding the result rather than only its causes. The strip is deliberately NOT applied to `--staged`: `git commit -- <pathspec>` builds a temporary index and hands the hook its path in `GIT_INDEX_FILE`, and that index — not `.git/index` — is the authoritative statement of what the commit will contain. Second, decoding a UTF-16 file as UTF-8 reads it *wrong* rather than failing on it: every NUL padding byte is itself valid UTF-8, so nothing is dropped and an ASCII payload arrives NUL-interleaved, matching no pattern in the catalogue. The vector is concrete rather than theoretical on a Windows-developed repository — PowerShell's `>` redirect and `Out-File` emit UTF-16LE by default, so a credential captured to a file and accidentally staged lands as tracked content the gate cannot see. The catalogue module gains a single `scan_bytes` entry point applying the patterns across every text reading the bytes admit (BOM-directed, plus a NUL-stripped pass that is the only thing able to catch BOM-*less* UTF-16), deduplicated across readings; the hook CLI's working-tree, staged and pushed-history readers and the CI gate all route through it. `scripts/secret_scan.py` had the narrower version of the same gap — it honoured a BOM but could not see UTF-16 without one — and gains the same second reading. Verified across all 4,900 tracked files with no new findings and no runtime change, so the extra readings cost nothing in precision here. Regression suite in `tests/test_leak_gate_blind_spots.py`, every test confirmed failing against the pre-fix code.
- **Thirteen output-branch disclosure asymmetries closed: a degraded run that the JSON envelope reported honestly, while the SARIF a CI gate consumes and the text a human reads both said clean.** Every one of the SARIF-blind entries in the W1331 ratchet is now gone. `test-impact --sarif` was the sharpest: with the default `--limit`, the cap-hit marker was assembled inside the `if json_mode:` branch alone, so a gate received the top-N slice of the impacted-test set with no truncation flag at all — measured on this repository, 2 of 972 reachable test files presented as the whole answer. `roam algo` folded its three warning buckets (malformed `.roamignore-findings`, malformed per-finding suppressions, unknown `--only`/`--exclude` names) together inside the JSON branch, so a suppression file that failed to parse disabled every suppression rule while SARIF and text reported an unannotated finding set. `dead`, `clones`, `duplicates`, `fan`, `n1`, `over-fetch`, `orphan-imports`, `critique` and `dark-matter` each floored a raised substrate call to an empty result and disclosed it only to `--json`; on the SARIF path that is an empty `results` array, which is exactly what a Code-Scanning gate reads as PASS. `dead` and `clones` additionally rendered the `empty_corpus_state` distinction — nothing indexed versus nothing found — in the JSON envelope only, so an unbuilt index printed the same all-clear as a clean repository. Each fix routes the existing bucket through `formatter.echo_text_warnings()` (and the new sibling `formatter.echo_text_empty_corpus()`) in the non-JSON tails: markers go to STDERR, so stdout stays byte-identical and no golden output moves. Every command was run before and after on a fixture where the signal genuinely fires, each with a clean control proving the disclosure stays silent on the happy path. The W1331 ratchet baseline drops 80 → 67 and its high-water mark with it.
- **Five commands absorbed a broken input into the value that means "fine", so a signal that could not be computed rendered as a signal that came back clean.** One defect shape, five instances, each proven with a fixture where the signal genuinely should have fired and a positive control on the same fixture with healthy input. `verify --checks tenant_scope`: an unparseable `.roam/verify.yaml` made the guard loader return an empty tuple, which is also what the deliberate "disable this detector" configuration returns — the detector switched itself off on a typo, dropping both the WARN finding and its `agent_contract.risks` entry with no disclosure anywhere. The loader now returns the guards *and* an error channel, and the two empty states are no longer confusable. `verify --report`: whole-repo target discovery returned an empty path list on any index-open failure, so a corrupt index scored `100/PASS` with `verification_complete: true` and `files_checked: 0` — while the non-report path on the identical database already failed loudly, so the two disagreed about the same file. Discovery failure now routes into the existing loud discovery-failure emitter (exit 5, `verification_complete: false`). `db-check`: three `OperationalError` paths were downgraded to `severity: "ok"`, so dropping a core metrics table reported verdict `OK`, `errors: 0`, and `--ci` exited 0; a check that cannot run is now `error` or the new non-clean `unsupported`, its count is `None` rather than a measured zero, and the verdict gains `INCOMPLETE`. Found while proving it: the zero-symbols check queried a column name that does not exist, so it raised on *every* database, healthy or not, and was unconditionally masked — that check had never once run. `roam algo` detectors: twelve per-detector `except sqlite3.Error: return []` handlers pre-empted the orchestrator, which already logs the failure and records it in `failed_detectors`; deleting them lets a detector whose query fails be reported as failed instead of as finding nothing. `service-report reachability-triage`: three security counters in the executive summary carried a `0` default while their own sibling sections rendered an em-dash for the same absent field, so a failed component printed "Taint flows: 0" in one section and "—" in another. Regression suite in `tests/test_w1332_uncomputable_signal_floor.py`, every test paired with a negative control so a fix that merely makes everything loud cannot pass, plus a structural guard that fails if the swallow-into-empty-list handler is reintroduced in the detector catalog.
- **`roam pr-prep <range>` scored the working tree instead of the range, and could clear a critical change set as safe.** `pr-prep` bundles `diff` + `critique` + `pr-risk` into one `ready_to_open` envelope. The `diff` sub-invocation forwarded the positional `commit_range`; the `pr-risk` one only ever forwarded `--staged`, so the two sections of a single envelope described different change sets. On a clean tree the un-scoped `pr-risk` resolves to `no-changes` and scores 0 — measured on roam's own repository, `pr-risk HEAD~20..HEAD` returns `79/100, "Critical risk … significant blast radius" (driver: blast_pct)` while the bare call `pr-prep` actually made returns `no-changes`, 0. `ready_to_open` is computed off exactly that score, so this was a false-clean rather than a cosmetic mismatch. Pre-existing since v12.17. The fix mirrors `diff`'s precedence (an explicit range wins over `--staged`); the regression tests assert at the argv boundary where the defect lived, rather than on a risk score, so they do not become a tuning tripwire — including one that requires `diff` and `pr-risk` to be scoped identically, since a reader takes two sections of one envelope as two views of one thing.
...
- **`roam --json savings --aggregate` — the platform savings privacy boundary** — emits a versioned, closed, aggregate-only contract with producer-authored state and verdict labels. The envelope excludes session, turn, episode, prompt, response, path, command, and generic agent-contract fields, so Prakteon can ingest readiness and outcome coverage without receiving local transcript content or per-episode identifiers.
- **Complete-evidence `roam verify` autofire contract** — changed-scope discovery now covers staged, unstaged, untracked, renamed, deleted, and unborn-HEAD worktrees; refreshes stale index targets; and reports explicit completeness, discovery, timeout, parsing, unavailable-check, and partial-success state. Any incomplete non-advisory check or malformed test protocol blocks the gate instead of producing a false pass. Claude Stop hooks consume the strict envelope, refuse edited-turn completion without complete evidence, and prohibit automatic fixes from changing policy, baselines, scopes, or suppressions.
- **Default-on execution-mode enforcement** — CLI and MCP write surfaces fail closed when policy loading or mode resolution fails, while read-only diagnostics retain a conservative fallback. Emergency overrides are explicit, audited, and reflected in MCP receipts; `verify` and replay-capable sibling-patch operations carry truthful side-effect classifications.
- **Hardened local telemetry and replay containment** — compile telemetry is explicit opt-in, closed-schema, size/time bounded, and owner-only from byte zero. Raw prompts, raw prompt hashes, session/turn identifiers, and compiler fingerprints are excluded; an optional opaque episode join key and keyed repo-local task fingerprint preserve outcome joins and repeat detection without exposing prompt text. The derived savings database is rebuilt as an exact owner-only projection with no on-disk SQLite journal, so bounded-source retention also removes stale rows. Sibling-patch replay uses literal argv, a scrubbed environment, isolated HOME/TMP roots, bounded process-tree termination, and explicitly discloses that this is process containment rather than a kernel sandbox.
- **Digest-bound release provenance** — the publish workflow builds wheel and sdist once, validates tag/package identity, binds both artifacts and the CycloneDX SBOM into a closed manifest, transfers immutable artifacts between privilege boundaries, uses PyPI Trusted Publishing, and verifies published PEP 740 provenance before attaching release evidence.
- **Private cross-project Procedure Foundry atlas** — transcript backfill v6 derives closed intent archetypes, action phases, exact/near-intent clusters, result-size buckets, time-to-action, directly observed friction counters (code slicing, shell replay, verification retry, search/inspect cycles, post-edit context recovery, structured-output post-processing, help lookup, and failure replay), per-sanitized-template attempts/failures/no-results/retries/result-volume aggregates, and closed failure-reason labels. Executables, subcommands, flags, compile procedures, artifact labels, prefetched facts, and probe names now cross persistence only through closed vocabularies; unsupported identifiers collapse to `other`. Search exit code 1 is classified as `no_results`, not a tool failure. Failure text is classified transiently and discarded. Rankings disclose likely scripted/eval contamination separately and use an organic-support estimate; associated time/tokens remain explicitly non-causal.
- **Universal repeatable `--select EXPR` JSON projection** — every command can now project its completed envelope with the dependency-free subset `.field`, `.field.nested`, `[N]`, and `[START:END]` (for example `roam complexity --select '.symbols[:5]'`). Projection happens only at final serialization, after the complete envelope is available to evidence sidecars, and `--select` implies `--json`.
- **`roam grep` bounded context packets** — `-C/--context` now returns live match windows and `--whole-symbol` returns deduplicated enclosing-symbol packets, bounded by `--max-packets` and `--max-packet-lines`. Every match links to its packet; packet truncation, unreadable source, unindexed symbols, and stale indexed spans are explicit. A stale whole-symbol span safely falls back to a live match window instead of showing the wrong function.
- **Capability intervention contracts and transition-test atlas** — `@roam_capability(displaces=(...))` declares which repeated-work families a command is designed to replace. Procedure Foundry AST-reads these hypotheses without importing the lazy command tree and emits `intervention_mappings` plus prospective transition tests. Declarations do not discount historical research priority: effectiveness remains unmeasured until explicit exposure, joined terminal outcomes, transition reduction, and outcome non-inferiority pass. The savings ledger now understands paired pre-delivery `intervention_assignment` and terminal `intervention_observation` events, session-clustered intent-to-treat assignment, contamination disclosure, delivery/adoption coverage, and a pre-registered promotion contract with power analysis, confidence intervals, effect-size floors, and outcome margins. Failure classes remain routing-ineligible until precision validation, and long no-edit candidates exclude unknown and passive-only research/review/planning/documentation intents.
...
- **Claude hook installation is a bounded control-plane update** — settings and hook bodies are size-bounded, strict-JSON parsed, protected from duplicate keys, symlinks, junctions, hard links, and pre-install generation changes, then written through the canonical content-bound durable atomic writer with exact-interpreter commands. Create-only publication is native no-replace; existing state uses a final generation precheck and atomic last-writer-wins replacement without rolling back later writers. Unsafe paths stop without touching redirected content; recognized prior bodies remain healable from a closed digest registry.
- **Mixed MCP query/write tools are gated per invocation** — catalog metadata continues to declare maximum callable effects, while the dispatch boundary binds the concrete call arguments. A query-only `roam_fan` call remains available in `read_only`; `persist=True` raises the call to `safe_edit`, emits a write receipt, and is blocked before execution at lower authority.
...
### Security
- **`roam taint --ci` reported SAFE and exited 0 on a repository containing unambiguous SQL injection and OS command injection.** `taint_engine._symbols_matching` only queries the `symbols` table for a rule's sources/sinks, and the Python indexer (`python_lang.py`) never materialises import-bound attribute chains (`request.args`) or external call targets (`cursor.execute`, `os.system`) as `symbols` rows — they are not local definitions, so `relations.py::resolve_references` has nothing to turn into an edge, and the forward-BFS pass had no nodes to connect. "0 findings" was therefore indistinguishable from "scanned and clean." Measured before/after on the shipped `sqli` rules-pack fixture (`--rules-pack sqli` matches 3 rules by substring: `python-sqli`, `java-sqli`, `php-laravel-sqli`): `EXIT=0`, `"No taint findings across 3 rule(s)"` → `EXIT=5`, 1 finding; the companion `command-injection` pack shows the same flip. `run_taint` gains an opt-in text-scan fallback (`project_root` kwarg, default `None` — byte-identical for every pre-existing direct call site) that anchors literal source/sink text occurrences to their enclosing function via already-indexed line ranges when a rule's DB-matched sources/sinks come back empty, plus a same-function co-occurrence pass for the "source and sink both in one handler" shape forward-BFS structurally can't express. Two precision guards keep it from over-firing: a parameterized-DB-call heuristic (`execute(sql, params)` stays clean, `execute(sql + input)` still flags) and dotted-names-only source matching (bare words like `data`/`payload` can't anchor to an unrelated local variable). `summary.anchor_coverage` (`rules_evaluated` / `rules_zero_anchors`) plus `partial_success` + `warnings_out` now make a rule that never resolved any anchor LOUD instead of folding into a clean verdict — closing W805-KKKK (language-mismatched rule packs silently reading as clean) as a side effect.
- **`audit-trail-verify` could not detect tampering with the most recent (tail) record, while its own help text claimed "tampering with any record ... breaks the chain."** `_verify_chain` validates record N via record N+1's `previous_record_hash` back-link; there is no N+1 for the last record, so the tail — the most attacker-valuable record, since it holds the final verdict — could be edited in place with no detection. Every append now also stamps a sibling `<trail>.head.json` file holding the tail line's own SHA-256, cross-checked at verify time. Backward compatible by construction: a trail with no head file (foreign, hand-built, or written before this fix) gets no tail check — exactly the pre-fix behaviour — so existing trails do not fail closed on a field they never had. New `summary.tail_protected` discloses which case applied. A follow-up in the same wave closed a related silent gap: the head-file write itself had swallowed a bare `OSError`, which would have left a trail with no tail protection and no record of why; it is now routed through `log_swallowed` so the failure is visible at write time, not only at audit time. This remains a hash chain, not a keyed signature: an attacker who rewrites the trail and its head file together still leaves no trace. Separately, `dogfood.yml`'s `--no-audit-trail` flag had disabled the very audit-trail and conformance checking the workflow advertises, silently printing `conformance: None/100` on every CI run; the flag is dropped so each run reports the fresh trail's real Article 12 conformance score.
- **`roam capsule --redact-paths` leaked the directory tree it exists to hide.** The flag correctly hashed `symbols[].file` and `clusters[].label`, but `symbols[].signature` and the `warnings_out` diagnostic accumulators were left untouched — a module-level string constant (`FILE = "src/roam/commands/cmd_hooks.py"`) or an exception message naming a real path landed verbatim in free text. A whole-envelope scan on roam's own repository (43,958 symbols) found 266+ signatures leaking a real path, plus one exception-message path, after the originally-reported field-level leak had already been closed. Per-field redaction is exactly the kind of choke point a new field bypasses by omission, so it is replaced with a single text-level scrub (`_scrub_text`/`_scrub_token`) that tokenizes free text and hashes any token matching a real indexed path component wherever it appears — including a filename glued to a sentence-final period — applied to `symbols[].signature` and both `warnings_out` marker accumulators. `--redact-paths` help text now states the guarantee precisely: it hashes known indexed path components in `file`/`label`/`signature`/`warnings_out`; it does not invent redaction for an out-of-corpus path, and it does not hash symbol names themselves. A new whole-envelope regression test scans the full serialised output against a distinctive fixture tree rather than checking one field at a time, which is how this class of leak survived the original fix.
- **Three defects together made the vulnerability-surface report the opposite of the truth.** `vuln-map` ingested reports without `project_root`, so the import scanner never ran and every genuinely-imported package showed "no match (not imported)" — measured 0/4 matched on the dogfood security fixture; the matched-count is now widened to include import-site evidence (not just resolved symbol ids, since third-party packages rarely have an indexed symbol), bringing it to 2/4. `vulns --reachable-only` post-filtered to `reachable==1`, which is always empty for import-site matches by design (they never seed a symbol id), and the verdict read that empty list as "table is empty / no scan available" — concealing 4 already-ingested CVEs; the pre-filter row count now carries through the verdict/summary so "no data" and "N rows, 0 reachable" are distinguishable in both text and JSON. Ingestion itself was a bare `INSERT` with no dedup, so re-importing the same report doubled the row count every time (4 → 8 on a second import, unbounded on further repeats), silently breaking any count-based CI gate; a NULL-safe UNIQUE index on `(cve_id, package_name, source)` plus an UPSERT that refreshes severity/title/match evidence on conflict makes re-import idempotent (stable at 4). `source` deliberately stays in the dedup key so two different scanners flagging the same CVE remain distinct findings rather than collapsing into one row, and a migration ledger pair was needed because a naive unique index would crash outright on a database that already holds duplicates from the pre-fix bug.

### Fixed
- **MCP setup and read-only security evidence now fail closed at their final trust boundaries** — JSON/TOML config publication is atomic, rejects linked primary and backup targets, and refuses a merge when the parsed config generation changed concurrently. The Codex adapter writes the current `~/.codex/config.toml` STDIO contract without clobbering unrelated tables. Receipts bind repository/run provenance, caller identity, and a deep JSON-safe argument snapshot before execution, so tool-side environment or nested-input mutation cannot rewrite the audited invocation. Read-only marker output cannot spoof the boundary-owned prompt-injection metadata key.
- **Public setup guidance and discovery metadata now track the live runtime contract** — generated preset counts include the always-registered expansion meta-tool, compliance exposes 14 tools, stale CI/help invocations are rejected by doc tests, redaction docs include `prompt_injection_marker`, and the MCP card discloses the optional bounded host-model summarization egress path.
...
- **The complete development/CI dependency graph is vulnerability-clean** — the dev-only Pillow floor is now 12.3.0, closing the image-parser advisory wave present in the previous 12.2.0 lock. Pillow remains outside Roam's runtime and publishing dependency graphs.
- **MCP transport/task dependency floor now includes the upstream security fixes** — the `mcp` and development extras require MCP Python SDK 1.28.1 or newer, and the release lock resolves 1.28.1. This closes the authenticated-principal task/session gaps and missing WebSocket Host/Origin validation reported against the previous 1.27.1 lock.
- **Architecture Guardian tolerates transient fresh-runner parser downloads without hiding real failures** — the scheduled index build retries `roam init --yes` up to three times with bounded backoff, then exits non-zero. This seals the observed one-shot `tree-sitter-language-pack` transport reset while keeping exhausted downloads visible and blocking.
- **Cold parser acquisition is cross-process safe and truthfully documented** — the grammar-pack floor is now 1.13.3 with a `<1.14` compatibility ceiling. That range serializes cache mutation across processes, publishes manifest/library files atomically, retains one checksum-addressed platform bundle, and lets later grammar loads reuse it locally. Four simultaneous cold-cache indexers pass on both Windows and native Linux. Public, security, privacy, legal, CI, smoke, and air-gap guidance now distinguishes local repository analysis from this one-time public parser-binary retrieval.
- **Hosted deployments can pin a root-owned parser cache without hijacking general XDG state** — `ROAM_TREE_SITTER_CACHE_DIR` accepts only a canonical absolute path and configures the grammar pack before availability or acquisition calls. `ROAM_TREE_SITTER_CACHE_SEALED=1` rejects missing grammars before the package's auto-downloading path and disables download retries for cache hits. This lets Prakteon serve an attested read-only parser generation while Claude, Codex, and application caches retain their normal writable locations.
- **Parser availability no longer collapses backend failure into “no findings”** — every optional tree-sitter boundary now uses the 1.13.3 networkless `has_language()` probe to distinguish an unsupported grammar from acquisition, cache, integrity, or runtime failure. Unsupported grammars retain their documented skip or regex fallback; required parser-backend failures remain loud. Deterministic tests pin the new package-specific exception hierarchy and all 40 non-regex Roam language mappings.
- **Transient first-use parser downloads have one bounded retry contract** — parser and language acquisition share two short deterministic retries for package `DownloadError` only. Checksum mismatches, unsupported grammars, runtime failures, and an exhausted download budget remain immediately or ultimately loud; an AST boundary test prevents parser consumers from bypassing the centralized policy.
- **The offline publish wheelhouse now matches the parser runtime floor** — the hash-locked release toolchain carries `tree-sitter-language-pack` 1.13.3 and its reviewed Linux wheel digest, and a regression gate binds that publish input to the package contract. Public installation, network-boundary, troubleshooting, and cancellation copy no longer advertises an unavailable Compile PyPI package, absolute zero-egress behavior, unsupported-grammar reinstalls, or an unresolved account dashboard.
- **The CLI network boundary is now explicit and test-pinned** — a canonical inventory documents cold parser acquisition, `version --check`, `metrics-push`, GitHub fetch/post paths, external stale-reference probes, keyless Cosign, optional listeners, and project-command inheritance. This replaces the false claim that `metrics-push` was the only outbound surface and records exact defaults, destinations, and payload classes.
- **MCP-card auto-rotation tests no longer mutate release files** — all four write-path tests now copy count inputs, card mirrors, and the SHA-256 pin into a per-test `--root` shadow. File-based pytest-xdist scheduling can no longer expose a transient test mutation to the real card-hash or mirror readers; a parallel regression asserts that the canonical card is byte-identical before and after the suite.
- **`service-report --type due-diligence` no longer deadlocks inside `clones`** — report components now cross a literal-argv subprocess boundary, so multiprocessing detectors never spawn from an in-process Click runner. Every component has a 180-second timeout, process-tree cleanup, a 32 MiB output ceiling, duplicate-key rejection, and an explicit failure envelope; the full gather also has one 240-second budget with cleanup reserve, so host contention degrades late evidence explicitly instead of overrunning the caller's five-minute bound. Partial/degraded components are named in both Markdown and JSON instead of collapsing into a successful-looking report. Cost-aware scheduling reserves `clones` an exclusive slot while overlapping compatible scans, improving the measured warm due-diligence run from 148 seconds serial to 109 seconds (about 26%).
- **Historical transcript mining now fails closed around private filesystem state and resource pressure** — `.roam`, the keyed fingerprint salt, source JSONL files, and the derived snapshot are depth-, row-, directory-, file-, event-, and byte-bounded and checked without following symlinks, junctions, or hard links. Persisted transcript CWD values are normalized, grouped, and HMACed lexically; they never trigger path resolution, existence checks, UNC/SMB access, or automount probes. The default scan keeps the newest 2,000 files per source under a 10,000-file global ceiling, applies global directory/entry budgets across every source, and applies aggregate ceilings of 512 MiB, 1,000,000 JSONL rows, and one 120-second deadline covering discovery plus processing. Selection, traversal, and aggregate-budget truncation are explicit. Snapshot construction serializes incrementally under a 32 MiB / 50,000-event ceiling; duplicate keys, invalid UTF-8, deep JSON, and same-size concurrent rewrites are rejected before any file contributes evidence. Keys are created owner-only with exclusive first-writer semantics and bounded concurrent-read retries. Windows cleanup is bound to the native file identity; platforms without a portable conditional unlink retain the private random tempfile and disclose that state instead of risking deletion of a replacement. Unsafe or changed paths produce a structured error envelope.
- **Savings materialization is bounded and failure-complete** — every source log is read from one no-follow descriptor and accepted only when size, identity, timestamps, link count, and owner-only state remain stable through the read. Physical rows and parse time are bounded even when rows are blank or invalid, including a decoder that crosses the deadline mid-row. The rebuild refuses oversized prior databases and sidecars before SQLite access, checks only the bounded current ID set against the prior projection, and caps the installed database at 64 MiB. Its fsynced SQLite artifact is bound to the producer identity, link count, metadata, and SHA-256 digest before all producer handles close. The installer reopens and re-proves that generation while retaining its descriptor through native publication; Windows denies sharing and renames the exact handle with `FileRenameInfo`, preventing a proved source pathname from being rewritten or replaced before publication. A generation that changes at the final native boundary is moved off the consumer pathname before the exclusive handle is released. Create-only publication remains native no-replace; existing state uses one final destination-generation precheck followed by serialized atomic last-writer-wins replacement, with no pathname-based post-install rollback that could clobber a legitimate later writer. Containment, lock-timeout, SQLite, and operational I/O failures return a canonical JSON error envelope.
- **Compile telemetry and cache warming now preserve their read/write contracts** — `compile-stats` validates owner-only state without repairing ACLs, rejects ambiguous/deep rows with explicit degraded-read counts, and keeps useful KPI verdicts alongside the degradation. Telemetry counts normalized unique prefetched facts, enforces 0..1 classifier confidence and real UTC dates, and opens an existing log once with no-follow/single-link validation before any ACL change or read. `compile-cache build` streams corpus and Git inputs under byte, line, path, task, and time ceilings and reports every partial input in its standalone verdict.
- **Service-report delivery and engagement persistence are fail-closed** — `service-report` and `pr-replay` now share one owner-only, no-follow, single-link, root-contained, interprocess-serialized engagement ledger implementation, so a concurrent record from either command cannot be erased by the other's bounded rewrite. History is retained under 2 MiB / 5,000-record ceilings. BSD/macOS descendant-proof limitations propagate into component health, Markdown, and JSON; a requested PDF that cannot be rendered is reported as an artifact failure and is never listed as delivered.
- **Release JSON is unambiguous and bounded at every network/artifact boundary** — the manifest, CycloneDX SBOM, and PyPI recovery response now reject duplicate object keys, hard-linked or redirected inputs, concurrent file replacement, and oversized payloads. Every workflow JSON download also has a 16 MiB transport ceiling, preventing last-key-wins interpretation and unbounded disk/parser input before publication.
...
- **A budget-truncated JSON envelope could silently drop its entire payload while `summary.partial_success` still read `false` — the same failure shape `dfe59667` fixed for `attestation`, but in the truncator's own disclosure step.** `roam cycles` on any repository whose envelope exceeded the JSON token budget is the concrete case: `_drop_fields_to_budget` deletes the whole non-preserved `cycles` list (it's the only payload field the command emits, so there's nothing smaller to drop instead), while `summary.cycle_count` and `summary.verdict` kept reporting the true, pre-truncation count. A consumer reading only `summary` saw a complete, successful result over a payload that had just been discarded, and a consumer defensively reading `data.get("cycles", [])` could not distinguish "0 cycles found" from "cycles found but discarded". The root cause: `_annotate_truncation` — the helper `budget_truncate_json` calls on every truncation, for every command that goes through `json_envelope` — stamped `truncated`, `budget_tokens`, and `omitted_low_importance_nodes` but never touched `partial_success`, which defaults to `false`. This broke an invariant the codebase already documents and tests elsewhere for the sibling `--limit` cap-disclosure path (`summary.partial_success : bool — True iff truncated`, see `tests/test_w1142_followup_b_cap_disclosure.py`). `_annotate_truncation` now sets `partial_success = True` whenever it fires, closing the gap for every command that goes through `budget_truncate_json` (both its callers — `json_envelope`'s own default-cap path and `mcp_server.py`'s explicit `--budget` path — share this one function), not just `cycles`. A separate, pre-existing truncation mechanism with the identical disclosure gap — `strip_list_payloads` / `_annotate_summary_disclosure`, used by the `--detail`-aware `clusters` / `dead` / `deps` / `health` / `hotspots` / `layers` / `smells` commands to elide list payloads in default (non-`--detail`) mode — was found during this audit still emitting `truncated: true` with `partial_success: false` on live repositories (confirmed on `roam --json health` and `roam --json layers`) and is deliberately NOT touched here: those seven commands already carry dozens of tests encoding a bespoke, per-command `partial_success` discipline (flip only on failure markers / non-empty `warnings_out`, never on the clean `--detail`-elided path), and reconciling that with a blanket "truncated implies partial_success" rule needs its own dedicated pass, not a drive-by inside this fix.
- **`summary.truncated: true` meant two different things depending on which of two independent mechanisms set it, with no field a consumer could branch on to tell them apart.** `budget_truncate_json` / `_annotate_truncation` sets it when the JSON token budget forced an involuntary payload drop (`summary.partial_success` correctly `true`, per `92a18361` above). `strip_list_payloads` / `_annotate_summary_disclosure` — used by the `--detail`-aware `clusters` / `dead` / `deps` / `health` / `hotspots` / `layers` / `smells` commands — sets the identical `truncated: true` when a default (non-`--detail`) invocation intentionally elided its list payloads, with `partial_success` correctly staying `false` (confirmed live: `roam --json health` and `roam --json layers` both emit `truncated: true`, `partial_success: false`). Both states are legitimate and `partial_success` is right in both cases — but a consumer that only sees `truncated: true` cannot tell "you didn't ask for detail" from "the budget ate your data" without inferring it from `partial_success` (itself ambiguous — `false` could mean "nothing dropped" or "intentionally elided") or from which command it called. Both mechanisms now also stamp `summary.truncation_reason`: `"budget"` for the involuntary JSON-budget path, `"detail_mode"` for the intentional `--detail`-elision path — a closed two-value enum a consumer can branch on directly, with `truncated: true` never appearing without one. `partial_success` semantics are unchanged for all seven `--detail`-aware commands and their existing tests. Separately, `pr-bundle init` unconditionally reset `summary.partial_success` to `false` after building its envelope, which would have silently re-clobbered a legitimate `true` set by the same JSON-budget path (e.g. a re-init over an already-large bundle); it now only defaults the flag when the truncator hasn't already raised it.
- **`roam attest --sign --format json` could emit a packet with no `content_hash` — no error, exit code 0 — on any repository whose envelope exceeded the JSON token budget. Affected releases: v13.7.0 through v13.9.0.** The envelope truncator preserved a fixed set of envelope fields (`command`, `summary`, `schema`, `schema_version`, `version`, `project`, `_meta`) and dropped every other field in dict insertion order until the payload fit the budget. `attestation` was not in that set, and it is the first payload field `attest` declares, so it was deleted *ahead of* the `evidence` block queued behind it: on Roam's own repository that meant 1,760 characters of signed contract discarded to make room for a 173,325-character blob which was then discarded anyway. The result is a tamper-evidence failure shaped like a success — `roam attest --sign --output packet.json` wrote a well-formed packet, printed its usual confirmation, and exited 0, with the seal simply absent. **If you hold a JSON attestation packet produced by v13.7.0–v13.9.0 from a repository large enough to truncate, check it for `attestation.content_hash` and re-issue it if that field is missing.** The `--format markdown` and default text outputs were never affected: both render the attestation directly, before the envelope is assembled, so only JSON artifacts lost the seal. The `stale_if` freshness contract, introduced in v13.7.1, was lost by the same mechanism over v13.7.1–v13.9.0, as was the bounded `agent_contract` block on every over-budget envelope of any command. `attestation` and `agent_contract` now join the preserved set — both are small, fixed-shape contract blocks that cannot themselves be what exceeds a budget, and `agent_contract` was already preserved on the sibling `strip_list_payloads` path — and the remaining fields are now dropped largest-serialised-first, so the field that actually blew the budget goes before smaller fields that merely sat in front of it.
- **A read-only report no longer states a measured zero when it never managed to read the file.** Three related defects made local compile/savings telemetry unreadable on the machine that wrote it, and made the resulting nulls indistinguishable from real observations. (1) The UserPromptSubmit hook body spawned its compiler as a bare `roam` resolved off `PATH`, so whichever install happened to shadow the active environment did the writing — a stale one writes rows in an older schema and never installs the owner-only state directory. The cold-compile path now resolves through the hook's own interpreter, matching how the hook itself is registered, and the hook-body version is bumped so already-wired installs heal. (2) `compile-stats` treated one weak property — whether the state directory's ACL is *exclusive* — as equivalent to the strong property that it is not *redirected*, and refused the whole read when it was merely inherited (the normal state of any repo predating the owner-only writer, or living under a group-writable parent). Redirection checks — symlink, junction/reparse point, hard link, non-regular file, size bound, and dev/ino identity across the open — remain hard failures; only ACL exclusivity degrades, and it degrades to an explicitly disclosed read state rather than to silence. The agent-facing fact now distinguishes "the log has N rows" from "the log was not read", instead of reporting `0 rows` over a populated file. (3) `savings` exited `RUN_FAILED` on that same ACL predicate, so "the evidence gate was not met" and "the evidence gate crashed" produced identical output and neither could be recorded. An ACL-only refusal is now an ordinary `insufficient_evidence` result carrying a per-conjunct breakdown of the seven-way admissibility gate, with each conjunct marked `unevaluated` rather than `failed`; redirection-shaped refusals still fail hard. The same breakdown is now the single source of truth for the gate when it *does* run, so every conjunct reports its observed value and threshold instead of collapsing into one boolean.
...
- **`changelog.html` (the landing-page mirror of `CHANGELOG.md`, deployed as-is by `wrangler pages deploy`) could drift and stay committed that way.** The render-drift check (`scripts/build_changelog_html.py`) ran only at push time and in CI — after a stale pair had already been committed — so the fix required a manual follow-up commit whenever anyone noticed (`5e8d6360` was exactly that, twice in one session). `.githooks/pre-commit` now runs the same check at commit time, so a `CHANGELOG.md` edit that doesn't also re-render the page is blocked before it lands.
- **`runs end` refused to record a `completed` status whenever any mirrored event reported `partial_success: true`, conflating incomplete OUTPUT with a FAILED check.** `_derive_status_from_recorded_checks` treated any `partial_success=true` event as a failed check, but `partial_success` was formalised (task #57 / `baa37ec9`) as an output-completeness signal, not a check verdict — confirmed across `cmd_pr_bundle.py` (`state != complete`), `cmd_verify.py` (execution degraded, never violation count), and `cmd_preflight.py` (degraded resolution, never risk). A plain `pr-bundle init` producing a not-yet-fully-assembled bundle document was enough to have the whole run refused; a budget-truncated command hitting the same envelope-truncation `partial_success` flag would have marked an entire run failed. `auto_log` now marks every `partial_success=true` it mirrors from a command's own envelope with `partial_success_kind: "output_incomplete"`; the status derivation excludes marked events from the failed-check set but still tracks them via `RunMeta.partial_output_count`, threaded into the signed `RunLedgerRoot/v1` predicate, so a verifier sees "completed, derived, AND N checks returned partial output" together rather than losing the signal. A raw/manual `partial_success=true` with no marker is still an explicit recorded failure and still refuses `completed` — preserving the original protection (closed hole `12c8b34d`). A separate bug in `read_run_meta`'s known-field allowlist, which silently dropped the new field into `extra` and let a stale dataclass default clobber it on the next `to_dict()` round trip, was found and fixed in the same commit.
- **`pr-analyze` was blind to a diff supplied via `--input`, stdin, `--diff-from-pr`, or `--staged` — only an explicit commit range reached the risk scoring underneath it.** `_capture_pr_prep` forwarded only `commit_range` to `pr-prep`; with `commit_range=None`, `pr-prep` silently recomputed its own diff from the working tree, flooring `pr_risk_score`/`high_severity_findings` and letting `pr-analyze` verdict SAFE on a diff that `critique` independently flags HIGH — a false negative on every diff-piping automation entrypoint, including `git diff | roam pr-analyze` and the Roam Agent Review `--diff-from-pr` path. `_acquire_diff` now reports which source it resolved, so `_capture_pr_prep` can distinguish "git-resolvable in this tree" (forwarded as before) from "acquired out-of-band"; external diffs are persisted to a temp file and threaded through a new `pr-prep --input` option. Since `diff`/`pr-risk` are git-native and cannot faithfully score a diff with no resolvable range in this tree, `pr-risk` is now SKIPPED with a loud `skipped: true` disclosure for that path — rather than silently scoring the ambient working tree — while the diff summary is built directly from the diff text. `--staged` now also reaches `pr-prep`'s diff/critique/pr-risk sub-invocations (it previously always analyzed unstaged even under `--staged`). Verified red against the unfixed code (captured `high_severity_critique` 0 vs. a standalone `critique --input` run's 1, contaminated by an unrelated dirty file) and green after. A known remaining gap, deliberately not fixed here to keep the blast radius small: `pr-prep`'s internal `pr-risk` sub-invocation still ignores its own `commit_range` argument.
- **`roam effects` counted `dict.get`, `set.add`, `dict.update`, and any `cursor.<SELECT>` as database operations because its patterns matched on method name alone, regardless of receiver.** `cursor.execute("SELECT ...")` was also misclassified as a write since the SQL verb was never inspected. Direct counts on roam's own repository: 6,939 `reads_db` and 2,028 `writes_db` before the fix; after gating `.execute(`/`.executemany(` on a literal SQL keyword and dropping the receiver-agnostic verbs (mirroring `side-effects`' import/call-edge-aware approach), the same repo reports 1,220 `reads_db` and 585 `writes_db` (-82% / -71%) with no loss on the real-DAO regression fixture. `roam side-effects` was already correct and had been contradicting `effects` the whole time.
- **`verify-imports` gated its stdlib skip on the HOST file's language, so a Python `import json` embedded in a `.yml` `python -c` heredoc was reported as an unresolved (hallucinated) import.** `_scan_import_entry` checked `is_py` — the host file's own language — before applying the known-stdlib skip; but `_extract_import_names_from_line` only falls back to Python-shaped regexes for a non-JS/non-Go host, so any name it pulls from a `.yml` file is Python-shaped by construction. On roam's own repository this false-positive class was measured at ~33% of all `verify-imports` findings. A new `stdlib_scope` parameter covers that case while real JS/TS/Go hosts stay on the old `is_py`-only gate, since their import names can coincidentally collide with a Python stdlib module name (an npm package called `os`, a Go `os`/`io` import) and already have their own correct resolution path.
- **`article-12-check`'s high-risk classifier had four independent defects that made its sample unreliable.** `_classify_high_risk_likelihood` matched bare `\bword\b` regexes over raw file text, so generic systems vocabulary — "promote" (cache promotion), "terminate" (process termination), "retention" (log retention) — tripped it with zero context, counting the same whether the match was in real code or a comment/docstring. Its test-file exclusion was `"test" in path`, a substring check that also matches `latest` and any path a user happens to check out under (including pytest's own tmp dir). It used `Path.cwd()` instead of the hardened `find_project_root()`. And its filesystem walk was unordered and capped at 200 files, so which 200 files got sampled varied run to run. The raw-text regex is replaced with an AST walk over genuine code identifiers (function/class/variable/attribute/parameter names) checked against a curated employment-domain vocabulary, requiring an ambiguous verb like `promote`/`terminate` to co-occur with an employment noun in the SAME identifier; the test-path check now routes through the canonical `roam.index.file_roles.is_test` detector; `Path.cwd()` is replaced with `find_project_root()`; and candidates are sorted before the 200-file cap so the sample is deterministic.
- **`find_project_root()` trusted any path that existed at `.git`, including an empty or stray directory left by an interrupted `git init` or a half-finished clone.** A stray empty `.git` in a tmp-dir ancestor silently rooted an entire pytest run at `/tmp` in one session, causing 214 test failures with no indication why — and the same shape breaks any user with a stray `.git` above their real project, since indexing, glob scope, rule scope, leak-gate scanning, and blast-radius all compute against the wrong tree. `_is_genuine_git_marker()` now validates the `.git` entry before trusting it: a directory needs `HEAD`/`objects`/`refs` (what `git init` creates immediately, even pre-first-commit); a file (worktree or submodule pointer) needs its contents to actually start with `gitdir:`. An invalid marker is skipped, not raised on, so a harmless leftover directory degrades gracefully instead of crashing, and an all-bogus ancestry still falls back to the start path rather than adopting anything. Filesystem-marker validation was chosen over shelling out to `git rev-parse` by measurement: ~150-200us added per call (0.85ms → 1.0ms) versus ~35ms per subprocess call on the same machine — with 160+ call sites, the subprocess would have been a bad trade for a hot-path helper.
- **A test-skip hook keyed on a substring match against a whole test module's source text — not a per-test marker — disabled 161 collected tests across 9 files whenever the private, gitignored dogfood corpus was absent, though only 7 of those tests genuinely read it.** `tests/conftest.py` skipped an entire module if its source merely contained the substring `"internal/dogfood"` anywhere — a docstring, a comment, or a mocked argument value — so on public CI or any outside clone, 154 tests that never touch the corpus never ran on the only environment where a stranger's change gets checked. Casualties included the leak-gate exemplar suite and the `describe --agent-prompt` Stack-line regression guard — the correctness suite for one of this project's own security gates was silently not running on the environment that matters most. Replaced with a `pytest.mark.needs_dogfood` marker, registered in `pyproject.toml` and applied only to the 7 tests that actually read the corpus; the conftest hook now skips only marked items. Measured: with the corpus hidden, the 154 previously-silently-skipped tests now execute; with it present, the same 165 tests (161 + 4 new guard tests) still pass.
- **Ruff formatting/lint was gated only at push time, over the whole working tree — so an unformatted file could land in a commit and block every subsequent push, including unrelated ones, until someone noticed and fixed it by hand.** This happened three times in one session on 2026-07-28: three separate agents each committed an unformatted file, and each one held up a 15+ commit release push for everybody. `.githooks/pre-commit` now also runs `ruff format --check` and `ruff check` on the STAGED Python files only, scoped deliberately the same way the leak and secret gates already scope to the pushed range rather than the whole tree — a commit-time gate should describe the commit.
...
- **`roam compile-stats --schema`** — documents the 17 `compile-runs.jsonl` telemetry row fields (name + one-line meaning, in writer order), text + `--json`; works with no telemetry log present.
...
- **`roam index --rebuild`** — forgiving hidden alias for `--force` (full reindex). A stray `roam index --rebuild` used to fail with `Error: No such option`; it is now accepted with identical behaviour. `--force` remains the canonical, documented spelling.

### Fixed
- **`roam auth-gaps` sees multi-line auth-middleware groups (Laravel comprehension)** — a Laravel `routes/api.php` wraps its app in `Route::middleware(['auth:sanctum', …])->group(function () { … })`; every route inside is authenticated. The brace-depth tracker counted `{`/`}` inside *comments and backticks* (route files write URL params like `{id}` in doc comments: `// registered before {id} …`), so a comment `}` dropped brace depth onto the auth group's boundary and **popped the whole group** — every route after it read as "missing auth". Measured on a real app: **433 false high-confidence auth gaps → the auth group is now tracked correctly** (5 → 61 controllers recognised as route-protected; 352 → 4 routes seen outside auth). Brace counting is now string/comment/backtick-aware with cross-line state (`_brace_deltas`). Genuine gaps outside the auth group still report.
- **`roam orphan-routes` composes `Route::prefix()` group paths** — `Route::prefix('reports')->group(fn () => Route::post('/bulk-send', …))` defines `/reports/bulk-send`, but the detector recorded the child path as `/bulk-send` (the `prefix` regex was defined but never called), so the frontend caller of `/reports/bulk-send` wasn't found and the route was flagged orphan. Prefixes now compose (nested groups included) via a string-aware brace matcher — route params inside a prefix string (`'billing-cycles/{cycle}/…'`) don't break parsing. Measured: ~50 prefix-composition false orphans on a real app.
- **`roam missing-index` no longer treats non-model classes as models** — a Seeder/Test/Console/Job/Observer/etc. class name was pluralised into a phantom table (`PayrollGlArticleSeeder` → `payroll_gl_article_seeders`, `DedupPerPeriodTest` → `dedup_per_period_tests`, `SendInvoiceJob` → `send_invoice_jobs`), and any `->where()` inside it produced a bogus "missing index" on that non-existent table. Model-table inference now skips non-model classes: test files (canonical `is_test_file`), `database/seeders/` + `*Seeder`, `Console/Commands/` + `*Command`, migrations, providers, and the PSR-4 queue/domain dirs `app/{Jobs,Observers,Policies,Listeners,Events,Mail,Notifications,Rules}/`. Matched by directory (not name suffix) so a real model that happens to end `Policy`/`Event`/`Job` (a CMS `PrivacyPolicy`, a `CalendarEvent`) is never gated, and an explicit `Model::where(...)` inside these classes still resolves — so no real finding is lost. Controllers and real Eloquent models still flag.
- **Brace/bracket matching is now PHP-source-aware across detectors (one shared scanner)** — several detectors scan source char-by-char to find a matching `}`/`]`; they counted delimiters that appear inside `//`/`#`/`/* */` comments and string literals. PHP writes URL params (`{id}`) in doc comments and strings, so a comment `}` drifted brace depth and mis-located the block: `roam over-fetch` mis-scoped Resource `toArray()` field arrays (a `]` in a string closed the array early → undercounted exposed fields) and method bodies; `roam migration-safety` mis-located the `up()` block (a comment brace could hide an unsafe `Schema::create` or leak `down()` in); `roam auth-gaps` popped its auth group (the 433-FP entry above). All three now share ONE scanner (`roam/commands/php_source_scan.py`) that skips strings (incl. multi-line, `\` escapes), `//`/`#` line and `/* */` block comments, backtick shell-exec strings, **heredocs/nowdocs** (`<<<SQL … SQL;` — an unpaired apostrophe in a heredoc body like SQL `-- don't` would otherwise flip a quote-aware scanner into string state and swallow the rest of the file), and treats PHP 8 `#[…]` attributes as code, not comments. Clean code is counted byte-identically to the naive count, so only the delimiter match got more accurate; a single implementation means the copies can't drift.
- **`roam dead` sees `X::class` references (Laravel `$casts` etc.)** — the PHP extractor emitted no reference edge for the `::class` constant expression, so a class referenced only via `['col' => MyCast::class]` (`$casts`, `$dispatchesEvents`, provider `$bindings`, config arrays) counted zero consumers and was flagged dead. A `class_constant_access_expression` handler now emits a reference to the class for the `X::class` form (skipping `self`/`static`/`parent`/`$obj::class` and non-`class` constants); genuinely-unreferenced classes still report.
- **`roam bus-factor` no longer flags build-artifact / non-source directories** — a real Laravel+Vue app reported `public/` (and `.vscode/`, `.idea/`, `out/`, `coverage/`, `generated/`, tool cache dirs) as CRITICAL bus-factor risk because their generated files have a single "author". These are now excluded by default (augmenting the shared `DEFAULT_EXCLUDE_PREFIXES` canon, component-aware so nested `packages/*/public/` is caught); `--include-excluded` restores the old behaviour. `.roamignore` paths were already dropped at index time.
- **`roam auth-gaps` no longer flags test methods as missing-authorization** — `tests/Feature/*ControllerTest::test_*` methods matched the `*Controller` file heuristic and their CRUD-word test names (`test_store_*`) read as high-confidence gaps. Findings in test files (`tests/`, `test/`, `spec/`, `__tests__/`, `*Test.php`, …) are now excluded by default via the canonical `is_test_file` classifier — filtered before persist, with an `excluded N test(s)` footer and an `--include-tests` opt-in. Genuine controllers (and a legitimately-named `TestController.php`) still report.
- **`roam verify-imports` no longer treats imports inside markdown code fences as real** — `.md`/`.mdx` files are excluded by default (`--include-docs` to opt in), so example `import` lines in documentation stop producing "unresolved import" findings.
- **`roam orphan-imports` resolves existing style-asset imports** — `import './styles.css'` / `'./theme.scss'` (also `.sass`/`.less`) with the file present on disk are no longer reported as `missing_local`; a genuinely-absent stylesheet still flags, so signal is preserved.
- **`roam dogfood-aggregate` lists failing paths inline** — the opaque `Parse failures: N (see --json for paths)` now prints each failing path one-per-line under the count, so the text output is actionable without re-running with `--json`.
...
- **Client-side pre-push history scans are exact and fail closed** — `.githooks/pre-push`
  captures Git's authoritative ref-update stream once, then scans newly referenced history
  without trusting mutable local remote-tracking refs. For new refs it queries Git's exact push
  destination and excludes only locally present commits that destination currently advertises;
  advertisements require exact fields, object IDs, and Git-compatible full ref names, while
  destination-query failures block instead of falling back to cached state. First pushes, tags,
  explicit/non-HEAD refspecs, deletions, force updates, and multi-ref pushes are covered;
  recursive per-parent traversal includes nested files, merge-resolution content, and Git type
  changes. Raw commit/tag objects, author/tagger metadata, published ref/path names, and direct
  blob/tree refs are inspected too; Git display-encoding settings cannot transcode findings away.
  Credential allowlists come from each immutable commit and must be regular files; placeholder
  exemptions recognize complete values instead of substrings, and an inline `secretsallow` marker
  must end the line. The internal-language gate scans every published path outside its exact
  documented whitelist without normalizing distinct Git names; direct-tree Gitlink names remain
  covered even though external submodule content does not publish here. Suppression comments apply
  only to eligible content, never metadata, and credential-bearing diagnostic labels are masked.
  All object reads disable replacement refs and capture bytes: identifiers and paths decode
  strictly, blob/object bodies
  decode loss-tolerantly with BOM-aware UTF-8/16/32 handling so malformed or wide text cannot hide
  later findings, and every Git/read/parse error blocks. The hook also verifies that `PYTHON`
  actually executes Python before trusting it. This keeps the gate complete under non-UTF-8
  Windows locales and prevents a later force-push from masquerading as remediation.
