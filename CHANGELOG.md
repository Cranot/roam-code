...
### Removed — BREAKING CLI surface

- **Three flags that were accepted from the user and then discarded are gone, and their removal is recorded as a break even though none of them ever reached behaviour.** `roam api-changes --changed`, `roam budget --staged` and `roam compile --model-tier` were each declared by a click decorator, bound into the command signature, and never read. A caller who passed one got silent acceptance and the unscoped result; the same caller now gets a click `UsageError` and **exit 2** — `Error: No such option: --changed`, `Error: No such option: --staged Did you mean --range?`, `Error: No such option: --model-tier`. That is a behaviour change on an unchanged command line, so it is written here rather than filed as a cleanup.

  **The no-op finding is measured, not asserted, because it is the whole basis for keeping the removal.** Against the pre-removal source bytes taken straight from git (`git show 2a95e2a0:<path>`, compiled rather than imported, so an editable install cannot shadow the result), each parameter is present in `co_varnames` and is the argval of **zero** load instructions in the command's own code object or in any nested one: `budget`/`staged` `LOADs=[]`, `api_changes_cmd`/`changed_only` `LOADs=[]`, `compile_`/`model_tier` `LOADs=[]`. The probe carries a positive control, without which a clean result would prove only that the probe is blind — `budget`/`explain`, `api_changes_cmd`/`base` and `compile_`/`artifact` are parameters that *are* read, and all three show `LOAD_FAST`. The remaining back-doors are closed by construction rather than by inspection: none of the three signatures has `**kwargs` or `*args` (`co_flags` VARKEYWORDS and VARARGS both false), none of the three parameters is a cell variable, so no nested function can capture it (`co_cellvars=()` on all three), and `git grep -n "ctx.params\|locals()" 2a95e2a0 -- src` exits 1 with zero matches, so there is no generic parameter back-door anywhere in the tree. A parameter that is never loaded cannot reach behaviour, which is why no revert is warranted for any of the three.

  **Remedies, one line each.** `api-changes` has no wider or narrower mode to select: its only file source is `git diff --name-only <base>`, so the narrowing `--changed` advertised was already unconditional and dropping the flag changes nothing but the error. `budget` compares two whole-tree metric snapshots and there is no staged-scoped metric to collect, so `--staged` scored the entire working tree while claiming to be scoped — drop it, and re-add it only alongside a real staged-content collector. `compile` selects an artifact on procedure and classifier confidence only, with no model-tier concept anywhere in the plan layer; pass `--artifact` to choose an envelope shape explicitly.

  **`dev/compatibility-baseline.json` was refreshed for these three entries and for nothing else, deliberately rather than silently.** The refresh went through the tool's own `--accept-removals` override, which exists precisely because regenerating a baseline until the diff disappears is the failure it is meant to make visible. The gate was **exit 5** before — `VERDICT: breaking changes (removed=3 renamed=0 added=0 breaking=3 coverage_gap=0)` — and the file diff is five lines: three flag entries deleted, plus one reflowed line where `"--range"` lost a trailing comma. Nothing else was added, nothing else was dropped, and `--require-coverage` still exits 0, so the baseline's reach was not reduced to make the count go down. Three is the whole of the CLI-surface removal here; the gate's own `removed=3` is the count this entry is accounting for, line for line.
...
**There are ZERO surface removals in this release.** `roam compatibility --json`, diffed against `dev/compatibility-baseline.json`, reports 0 removed commands, 0 removed flags, 0 removed envelope fields, 0 removed MCP tools and 0 breaking entries, and exits 0. Nothing was renamed, no flag was retired, no MCP tool disappeared, and no MCP preset narrowed. If you are looking for the API you have to migrate off, there isn't one.

The major is BEHAVIOURAL, and it is one mechanism repeated across the fixes below: **a check that could not run used to report that it had run and found nothing.** An absent measurement resolved to a benign value instead of to *unknown* — `()`, `[]`, `count=0`, `severity="ok"`, `score=100`, `VERDICT: ... clean` — so a gate that never executed came back green. Six of the fixes share a sharper variant worth stating on its own: *a value crossed a boundary without the identity of the producer that made it, and the receiver's default for "producer identity ABSENT" was "producer identity MATCHES" rather than "unknown".* Every fix converts an implicit equality, or an implicit pass, into an explicit UNKNOWN that fails closed. The consequence for you is that runs which exited 0 can now exit non-zero on the same inputs, on the same command line, with no flag changed — and that is the whole of the break.
