...
from roam.cli import _CATEGORIES, _COMMANDS  # type: ignore[import-not-found]
from roam.cli import _short_help_via_ast
...
def _build_appendix() -> str:
    ...
    lines.append(
        '  <p>Auto-generated from <code>roam --help</code>. Every canonical command + alias.</p>'
    )
    ...
    for category, names in _CATEGORIES.items():
        ...
        for name, help_text in rows:
            lines.append(
                f'      <tr><td><code>roam {_escape(name)}</code></td>'
                f'<td>{_escape(help_text)}</td></tr>'
            )
...
def main() -> int:
    target = ROOT / "docs" / "site" / "command-reference.html"
